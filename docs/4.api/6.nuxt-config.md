...
::note
相对路径（如 `./`）由于 Nitro 的限制，不能直接在 `nuxt.config.ts` 中使用。对于使用相对资源路径的静态托管，请使用以下方法之一：
...
**参见**：[Nuxt View Transition API 文档](/docs/4.x/getting-started/transitions#view-transitions-api-experimental)
...
目前，服务器会打印钩子名称和计时信息，浏览器会打印钩子参数。也可以设置为对象，以启用特定的调试选项。
...
**参见**：[Nuxt 文档](/docs/4.x/directory-structure/app/composables)

### `dirs`

自定义自动导入目录的数组。请注意，此选项不会覆盖默认目录（`~/composables`、`~/utils`）。
...
是否扫描 `app/composables/` 和 `app/utils/` 目录中的组合式函数以进行自动导入。Nuxt 或其他模块注册的自动导入（例如从 `vue` 或 `nuxt` 导入的内容）仍会保持启用。
...
配置用于路径解析的模块目录（例如 webpack 的 `resolveLoading`、`nodeExternals` 和 `postcss`）。

路径相对于 `options.rootDir`（默认为当前工作目录）。
使用 yarn workspace 类型的 mono-repo 时，可能需要进行配置。
...
::callout
**注意**：插件也会从 `~/plugins` 目录自动注册，无需在 `nuxt.config` 中列出，除非需要自定义加载顺序。插件按照其 src 路径去重。
::
...
::callout
**注意**：Nuxt 配置中只能传递 JSON 可序列化的选项。想要更精细控制，可使用 `router.options.ts` 文件。
::
...
`"nitro"` 和 `"vite"` 是 `@nuxt/nitro-server` 和 `@nuxt/vite-server` 的简写。后者具有实验性，仅使用 Vite 实现：它构建客户端且不提供自己的服务器，因此在 `ssr: false` 时会生成静态 SPA。启用 SSR 时仍会构建服务器环境，但运行它需要由 Vite 插件提供自己的目标，或交由自定义服务器完成。服务器路由、路由规则和预渲染不可用，需要服务器的模块也无法运行。
...
TypeScript 提供更严格的安全检查。转换完项目后，可以启用它以获得更高的安全性。
...
::warning
运行时模板编译会将编译后的模板作为 JavaScript 执行。切勿编译来自用户提供或其他不受信任输入的模板；这样做等同于 `eval`，并可能导致跨站脚本攻击或远程代码执行。请参阅 [Vue 安全指南](https://vuejs.org/guide/best-practices/security.html)。
::
