...
你可以通过第二个参数中提供的 `signal` 来使你的 `handler` 函数可中止。当请求不再需要（例如用户离开页面时）时，这非常有用。`$fetch` 原生支持中止信号。
...
当多次调用 `useAsyncData` 使用相同键时，它们会共享同一个 `data`、`error`、`status` 和 `pending` ref。这确保了跨组件的数据一致性，但要求使用的选项也保持一致。
...
- `status`：表示数据请求状态的字符串：
  ...
  - `success`：请求成功完成
  - `error`：请求失败
- `pending`：一个 `Ref<boolean>`，在请求进行时为 `true`。
