...
::note
你不需要对 `useAsyncData` 使用 `await`。在服务器端，无论是否 `await`，Nuxt 都会等待 Promise 解析后再进行渲染，因此返回的 HTML 始终包含数据。`await` 影响的是调用之后的行为：使用它时，执行会暂停直到 `data` 被填充，并且客户端导航会被阻塞直到数据准备就绪；不使用它时，执行会立即继续，`data` 会先保持默认值，直到请求解析完成，而在客户端导航中，你需要使用返回的 `status` 和 `error` refs 自己处理加载和错误状态。这与 [`lazy`](#parameters) 选项有类似效果，不过 `lazy` 才是显式选择非阻塞导航的方式。
::
...
```
只有当 `nuxt.config` 中启用了 `experimental.payloadExtraction` 时，才会缓存数据。
...
当多个 `useAsyncData` 调用使用相同的 key 时，它们会共享相同的 `data`、`error`、`status` 和 `pending` refs。请保持下面列出的选项在这些调用之间一致。

以下选项在所有使用相同 key 的调用之间**必须保持一致**：
- `handler` 函数
- `deep` 选项
- `transform` 函数
- `pick` 数组
...
这个组合式函数返回一个可被 `await` 的 `Promise`，这使得可以在 `<script setup>` 中直接使用 `data`（也就是说，此时会有值，而不是 `undefined`）。你也可以在不等待返回值的情况下直接解构这些值，在这种情况下，直到获取完成之前，`<script setup>` 中的 `data` 都可能是 `undefined`。
