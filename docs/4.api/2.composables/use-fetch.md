...
|---------------------------------------------------------------------------|-------------------------------------------------------------------------|------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `key`                                                                     | `MaybeRefOrGetter<string>`                                              | auto-gen   | 用于去重的唯一键。如果未提供，则根据 URL、选项以及源码中的调用位置生成。                                                                                                                                                                                                         |
...
| `watch`                                                                   | `MultiWatchSources \| false`                                            | -          | 要监听并自动刷新的一组响应式源。`false` 会禁用监听。                                                                                                                                                                                                                              |
| `deep` :badge[v3.8]{color="info" size="xs" class="align-middle"}          | `boolean`                                                               | `false`    | 以深层 ref 对象返回数据。默认值为 `false`，以提升性能（浅层 ref 对象）。                                                                                                                                                                                                         |
| `dedupe` :badge[v3.9]{color="info" size="xs" class="align-middle"}        | `'cancel' \| 'defer'`                                                   | `'cancel'` | 避免同一键同时被多次获取。                                                                                                                                                                                                                                                         |
| `enabled` :badge[v4.5]{color="info" size="xs" class="align-middle"}       | `boolean`                                                               | `true`     | 控制请求是否允许执行的门控条件。为 `false` 时，所有执行都会被阻止（初始获取、`execute`/`refresh` 以及 watch 触发），并且将 `true` → `false` 会取消任何进行中的请求，但不会清空 `data`。重新启用不会自动重新获取。 |
