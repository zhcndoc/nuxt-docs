...
::warning
等待 `useLazyFetch` 会初始化调用，但不会等待数据返回。在客户端侧导航期间，在使用结果之前，请在组件模板中检查 `status === 'pending'` 和 `status === 'error'`。
::
...
| `clear`   | `() => void`                                        | 将 `data` 重置为 `undefined`，将 `error` 重置为 `undefined`，将 `status` 设为 `idle`，并取消任何挂起的请求。 |
