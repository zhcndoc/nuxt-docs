...
`useHydration` 是一个内置组合式函数，提供了一种在每次新的 HTTP 请求时，在服务器端设置数据，并在客户端接收该数据的方式。这样 `useHydration` 允许你完全控制水合周期。
...
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| `key` | `string` | 用于在 Nuxt 应用中唯一标识数据的键。 |
| `get` | `() => T` | 仅在服务器端执行的函数（SSR 渲染完成时调用），用于设置初始值。 |
| `set` | `(value: T) => void` | 仅在客户端执行的函数（Vue 实例创建时调用），用于接收数据。 |
