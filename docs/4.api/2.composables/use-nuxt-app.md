...
<!--
note
默认情况下，Nuxt 的共享运行时上下文位于 [`buildId`](/docs/4.x/api/nuxt-config#buildid) 选项下的命名空间中。它支持多个运行时上下文。

## 参数

- `appName`：可选的应用名称。如果你不提供，则使用 Nuxt 的 `buildId` 选项。否则，它必须与现有的 `buildId` 匹配。 -->
...
#### 自定义 Reducer/Reviver :badge[v3.4]{color="info" size="xs" class="align-middle"}
...
/**
 * 这种类型的插件运行得非常早，在 Nuxt 生命周期中，甚至在我们恢复 payload 之前。
 * 你将无法访问 router 或其他 Nuxt 注入的属性。
 *
 * 请注意，“DateTime”字符串是类型标识符，在 reducer 和 reviver 中
 * 必须保持一致。
 */
...
export default defineNuxtRouteMiddleware(async (to, from) => {
  ...
  if (!user) {
    // 将正确的 Nuxt 上下文应用到我们的 `navigateTo` 调用。
...
// Vue / Nuxt 在调用 setup() 时会在 _vueInstance 中设置一个引用当前组件的全局变量
...
- `appName`：可选的应用名称。如果不提供，则使用 Nuxt 的 `buildId` 选项。否则，它必须与现有的 `buildId` 匹配。 -->