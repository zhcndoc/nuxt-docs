...
/**
 * 这种类型的插件会在 Nuxt 生命周期很早期运行，在我们恢复 payload 之前。
 * 你将无法访问路由器或其他由 Nuxt 注入的属性。
 *
 * 请注意，“DateTime”字符串是类型标识符，且在 reducer 和 reviver 中必须保持一致。
 */
...
// Vue / Nuxt 在调用 setup() 时会设置一个指向当前组件的全局变量到 _vueInstance 中
async function setup () {
  getCurrentInstance() // 可用
  await someAsyncOperation() // Vue 会在同一 tick 中、异步操作之前取消设置上下文！
...
- `appName`：可选的应用名称。如果你不提供它，将使用 Nuxt 的 `buildId` 选项。否则，它必须与现有的 `buildId` 匹配。 -->