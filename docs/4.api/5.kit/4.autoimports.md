...
::note
这些函数用于注册你自己的工具、composables 和 Vue API。对于页面、组件和插件，请参考相应章节：[页面](/docs/4.x/api/kit/pages)、[组件](/docs/4.x/api/kit/components)、[插件](/docs/4.x/api/kit/plugins)。
::
...
::tip
要为 Nitro 服务器上下文添加导入，请参阅 [`addServerImports`](/docs/4.x/api/kit/nitro#addserverimports) 函数。
::
...
| 属性      | 类型                  | 必需   | 描述                                                           |
| --------- | --------------------- | ------ | -------------------------------------------------------------- |
| `dirs`    | `string \| string[]`  | `true` | 指向要导入的目录的路径，字符串或字符串数组。                   |
| `options` | `{ prepend?: boolean }` | `false`| 传递给导入的选项。如果将 `prepend` 设置为 `true`，则这些导入将被添加到导入列表的前面。|
...
|-----------|------------------------------------------------------|----------|---------------------------------------------------------------------------|
| `from`    | `string`                                             | `true`   | 要从中导入的模块说明符。                                                   |
| `imports` | `(NuxtImportEntry \| NuxtImportPreset)[]`{lang="ts"} | `true`   | 条目数组，可以是导入名称、导入对象或嵌套预设。                               |
