...
| 属性      | 类型      | 必需 | 描述                                                                                                   |
|-----------|-----------|------|--------------------------------------------------------------------------------------------------------|
| `dev`     | `boolean` | `false`  | 如果设置为 `true`，则在开发模式下构建时会调用回调函数。                                                 |
| `build`   | `boolean` | `false`  | 如果设置为 `true`，则在生产模式下构建时会调用回调函数。                                                 |
| `server`  | `boolean` | `false`  | 如果设置为 `true`，则在构建服务端 bundle 时会调用回调函数。                                             |
| `client`  | `boolean` | `false`  | 如果设置为 `true`，则在构建客户端 bundle 时会调用回调函数。                                             |
| `prepend` | `boolean` | `false`  | 如果设置为 `true`，则会使用 `unshift()` 将回调函数添加到数组前面，而不是使用 `push()`。 |

## `setBuildOutput`

为给定的键设置一个构建输出提供者。构建输出是构建器（Vite、webpack、Rspack 或自定义构建器）与 Nitro 服务端运行时之间的契约：每个键都会映射到一个 `nuxt/*` 子路径导入，服务端运行时会在构建时解析它。这通常在构建器内部，或在参与构建的模块中调用。
...
**`key`**：构建输出键。可选值之一为 `serverEntry`、`clientManifest`、`clientPrecomputed`、`ssrStyles`、`entryChunkName` 或 `entryIds`。

**`provider`**：该键对应的值：一个（可能是异步的）函数，返回模块主体字符串；当服务端构建解析相应的 `nuxt/*` 导入时，会按需读取。
