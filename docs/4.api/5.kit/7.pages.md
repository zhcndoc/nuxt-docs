...
| 属性       | 类型                               | 必填     | 描述                                                                                     |
| ---------- | ---------------------------------- | -------- | ---------------------------------------------------------------------------------------- |
| `name`     | `string`                           | `false`  | 路由的名称。适用于编程式导航和标识路由。                                                 |
| `path`     | `string`                           | `false`  | 路由 URL 路径。如果未设置，Nuxt 将根据文件位置推断。                                     |
| `file`     | `string`                           | `false`  | 应作为路由组件使用的 Vue 文件路径。                                                     |
| `meta`     | `NuxtPageMeta`{lang="ts"}          | `false`  | 路由的自定义元数据。可用于布局、中间件或导航守卫。                                       |
| `alias`    | `string[] \| string`{lang="ts"}    | `false`  | 路由的一个或多个别名路径。适用于支持多个 URL。                                           |
| `redirect` | `RouteLocationRaw`{lang="ts"}      | `false`  | 路由的重定向规则。支持命名路由、对象或字符串路径。                                     |
| `children` | `NuxtPage[]`{lang="ts"}            | `false`  | 此路由下用于布局或视图嵌套的嵌套子路由。                                                 |
