...
| ---------- | ---------------------------------- | -------- | ---------------------------- |
| `name`     | `string`                           | `false`  | 路由名称。适用于程序化导航和路由标识。 |
| `path`     | `string`                           | `false`  | 路由 URL 路径。如果未设置，Nuxt 将根据文件位置推断。 |
| `file`     | `string`                           | `false`  | 应作为该路由组件使用的 Vue 文件路径。 |
| `meta`     | `NuxtPageMeta`{lang="ts"}          | `false`  | 路由的自定义元数据。可用于布局、中间件或导航守卫。 |
| `alias`    | `string[] \| string`{lang="ts"}    | `false`  | 该路由的一个或多个别名路径。适用于支持多个 URL。 |
| `redirect` | `RouteLocationRaw`{lang="ts"}      | `false`  | 路由的重定向规则。支持命名路由、对象或字符串路径。 |
| `children` | `NuxtPage[]`{lang="ts"}            | `false`  | 此路由下用于布局或视图嵌套的子路由。 |

### 自定义页面元数据的类型定义

你可以扩展 `NuxtPageMeta` 接口，以便在使用 `extendPages` 或 `pages:extend` 钩子时，为页面添加类型安全的自定义元数据：
  ...
  setup () {
    extendPages((pages) => {
      for (const page of pages) {
        // requiresAuth 现在的类型是 boolean | undefined
...
**options**: 一个要传递给路由配置的对象。如果设置了 `override` 为 `true`，它将覆盖现有的路由配置。
...
export default defineNuxtRouteMiddleware((to, from) => {
  // isAuthenticated() 是一个用于验证用户是否已认证的示例方法
