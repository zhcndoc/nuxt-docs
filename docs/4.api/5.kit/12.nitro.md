...
Nitro 是一个用于构建超快速 Web 服务器的开源 TypeScript 框架。Nuxt 使用 Nitro 作为其服务器引擎。您可以使用 `useNitro` 访问 Nitro 实例（如果您的模块也应在没有 Nitro 的情况下工作，则使用 `tryUseNitro`），使用 `addServerHandler` 添加服务器处理程序，使用 `addDevServerHandler` 添加仅在开发模式下使用的服务器处理程序，使用 `addServerPlugin` 添加插件以扩展 Nitro 的运行时行为，以及使用 `addPrerenderRoutes` 添加要由 Nitro 预渲染的路由。
...
返回 Nitro 实例；如果不存在，则返回 `undefined`。

在 `ready` 钩子运行之前不存在 Nitro 实例，并且当配置的 [`server.builder`](/docs/api/nuxt-config#builder-1) 不使用 Nitro 时，始终不会有 Nitro 实例，例如输出仅客户端 SPA 的构建器。在任何应当能够在没有服务器的情况下继续工作的场景中，请使用此函数，而不是 `useNitro()`：在这种情况下，服务器路由、路由规则和预渲染都不存在。
...
::warning
您必须在插件文件中显式地从 `nitro` 导入 `definePlugin`。对于 `useRuntimeConfig` 等工具，也有相同的要求。
::
...
export default definePlugin((nitroApp) => {
  nitroApp.hooks.hook('request', (event) => {
    console.log('收到请求', event.req.url)
  })

  nitroApp.hooks.hook('response', async (res) => {
    console.log('响应时', await res.text())
  })
})
```
...
::warning
如果你想提供一个可在服务器和客户端环境中使用的工具，并且可以在 [`shared/`](/docs/4.x/directory-structure/shared) 目录中使用，该函数必须在 [`addImports`](/docs/4.x/api/kit/autoimports#addimports) 和 `addServerImports` 中都从同一个源文件导入，并且应具有相同的签名。该源文件不应导入任何特定于上下文的内容（例如 Nitro 上下文、Nuxt 应用上下文），否则可能会在类型检查期间导致错误。
::
