...
Nitro 是一个用于构建超快速 Web 服务器的开源 TypeScript 框架。Nuxt 使用 Nitro 作为其服务器引擎。您可以使用 `useNitro` 访问 Nitro 实例（如果您的模块也应该在没有 Nitro 的情况下运行，则使用 `tryUseNitro`），使用 `addServerHandler` 添加服务器处理程序，使用 `addDevServerHandler` 添加仅在开发模式下使用的服务器处理程序，使用 `addServerPlugin` 添加插件以扩展 Nitro 的运行时行为，以及使用 `addPrerenderRoutes` 添加由 Nitro 预渲染的路由。
...
返回 Nitro 实例，如果不存在则返回 `undefined`。

在 `ready` 钩子运行之前不存在 Nitro 实例；当配置的 [`server.builder`](/docs/api/nuxt-config#builder-1) 不使用 Nitro 时，也完全不存在 Nitro 实例，例如输出仅客户端 SPA 的构建器。在任何应当能够在没有服务器的情况下继续运行的场景中，请使用此函数而不是 `useNitro()`：在这种情况下，服务器路由、路由规则和预渲染都不存在。
