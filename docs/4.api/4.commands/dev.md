...
| `--envName`                          |         | 在解析配置覆盖项时使用的环境（构建时默认是 `production`，运行开发服务器时默认是 `development`）                                                         |
| `-e, --extends=<layer-name>`         |         | 继承一个 Nuxt 层                                                                                                                                      |
| `--clear`                            | `false` | 重启时清空控制台                                                                                                                                      |
| `--no-f, --no-fork`                  |         | 禁用 fork 模式                                                                                                                                       |
| `-p, --port`                         |         | 要监听的端口（默认：`NUXT_PORT \|\| NITRO_PORT \|\| PORT \|\| nuxtOptions.devServer.port`）                                                            |
| `-h, --host`                         |         | 要监听的主机（默认：`NUXT_HOST \|\| NITRO_HOST \|\| HOST \|\| nuxtOptions.devServer?.host`）                                                           |
...
| `--profile` :badge[v4.4]{color="info" size="xs" class="align-middle"} |         | 分析性能。退出时写入 V8 CPU 配置文件和 JSON 报告。使用 `--profile=verbose` 可获得完整的控制台报告。                                                     |
