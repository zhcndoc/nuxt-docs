...
::note
为方便起见，在预览模式下，你的 [`.env`](/docs/4.x/directory-structure/env) 文件会被加载到 `process.env` 中。（不过在生产环境中，你需要自行确保环境变量已设置。例如，在 Node.js 20+ 中，你可以通过运行 `NODE_ENV=production node --env-file .env .output/server/index.mjs` 来启动你的服务器。）
::
