...
::note
`navigateTo` 不能在 Nitro 路由中使用。要在 Nitro 路由中执行服务器端重定向，请改用 [`sendRedirect`](https://h3.zhcndoc.com/utils/response#redirectlocation-status-statustext)。
::
...
::note
调用 `navigateTo` 不会停止执行其余的 `<script setup>` 代码。如果你希望成功导航后提前从 `setup()` 返回，请启用 [`experimental.navigateToEarlyReturn`](/docs/4.x/guide/going-further/experimental-features#navigatetoearlyreturn)。
::
...
- `redirectCode`
  ...
  - 当重定向在服务器端发生时，`navigateTo` 会重定向到给定路径并将重定向状态码默认设置为 [`302 Found`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/302)。

    可以通过提供不同的 `redirectCode` 来修改此默认行为。常见的做法是在永久重定向时使用 [`301 Moved Permanently`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/301)。
