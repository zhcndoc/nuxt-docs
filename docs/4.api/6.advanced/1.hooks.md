...
| `page:view-transition:start` | `transition`        | 客户端          | 当调用 `document.startViewTransition` 后调用（当启用了[实验性视图过渡支持](/docs/4.x/getting-started/transitions#view-transitions-api-experimental) 时）。`transition` 参数是具有 `types` 属性的 [`ViewTransition`](https://developer.mozilla.org/en-US/docs/Web/API/ViewTransition) 对象（[`ViewTransitionTypeSet`](https://developer.mozilla.org/en-US/docs/Web/API/ViewTransitionTypeSet)），可读可修改。 |
