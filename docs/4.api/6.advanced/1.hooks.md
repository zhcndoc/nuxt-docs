...
| `vue:setup`                  | -                   | Server & Client | 当初始化 Nuxt 根时调用。此回调必须是同步的。                                                                                                                       |
| `vue:error`                  | `err, target, info` | Server & Client | 当 vue 错误向上传播到根组件时调用。[了解更多](https://vuejs.org/api/composition-api-lifecycle#onerrorcaptured)。                                              |
| `app:rendered`               | `renderContext`     | Server          | 完成 SSR 渲染后调用。                                                                                                                                                    |
...
| `app:suspense:resolve`       | `appComponent`      | Client          | 在 [Suspense](https://vuejs.org/guide/built-ins/suspense#suspense) 解析事件发生时调用。                                                                                                    |
| `app:manifest:update`        | `{ id, timestamp }` | Client          | 检测到你的应用有更新版本时调用。                                                                                                                            |
| `app:data:refresh`           | `keys?`             | Client          | 当调用 `refreshNuxtData` 时调用。                                                                                                                                              |
| `link:prefetch`              | `to`                | Client          | 当观察到 `<NuxtLink>` 发生预取时调用。                                                                                                                              |
| `page:start`                 | `pageComponent?`    | Client          | 在 `NuxtPage` 内部的 [Suspense](https://vuejs.org/guide/built-ins/suspense#suspense) pending 事件上调用。                                                                         |
| `page:finish`                | `pageComponent?`    | Client          | 在 `NuxtPage` 内部的 [Suspense](https://vuejs.org/guide/built-ins/suspense#suspense) resolved 事件上调用。                                                                        |
| `page:loading:start`         | -                   | Client          | 当开始进行路由导航（在解析之前）或当页面 key 发生变化时调用。如果页面被复用（例如在 `definePageMeta` 中使用静态 `key`），在页面组件的 `setup()` 不会重新运行的情况下也可能触发。 |
| `page:loading:end`           | -                   | Client          | 在 `page:finish` 之后调用                                                                                                                                                            |
| `page:transition:finish`     | `pageComponent?`    | Client          | 在页面过渡 [onAfterLeave](https://vuejs.org/guide/built-ins/transition#javascript-hooks) 事件之后调用。                                                                            |
| `dev:ssr-logs`               | `logs`              | Client          | 使用从服务器传递到客户端的日志数组调用（如果启用了 `features.devLogs`）。                                                                      |
| `page:view-transition:start` | `transition`        | Client          | 当启用了[实验性的 viewTransition 支持](/docs/4.x/getting-started/transitions#view-transitions-api-experimental) 后、在调用 `document.startViewTransition` 之后调用。`transition` 参数是一个带有 `types` 属性的 [`ViewTransition`](https://developer.mozilla.org/en-US/docs/Web/API/ViewTransition) 对象（[`ViewTransitionTypeSet`](https://developer.mozilla.org/en-US/docs/Web/API/ViewTransitionTypeSet)），该属性可以读取或修改。 |
