...
:read-more{title="Nuxt 无障碍访问" to="/docs/4.x/guide/best-practices/accessibility#links"}

## Internal Routing

在此示例中，我们使用 `<NuxtLink>` 组件链接到应用中的另一个页面。
...
该组件会根据其文件名自动导入，因此你可以像平常一样使用 `<MyNuxtLink />`，并应用你的新默认值。`componentName` 只会设置组件的内部名称（如 Vue DevTools 中所示）；它不会改变组件在模板中的使用方式。
...
- `activeClass`：应用于激活链接的默认类名。其行为与 [Vue Router 的 `linkActiveClass` 选项](https://router.vuejs.org/api/interfaces/routeroptions#linkActiveClass-) 相同。默认为 Vue Router 的默认值（`"router-link-active"`）
- `exactActiveClass`：应用于精确激活链接的默认类名。其行为与 [Vue Router 的 `linkExactActiveClass` 选项](https://router.vuejs.org/api/interfaces/routeroptions#linkExactActiveClass-) 相同。默认为 Vue Router 的默认值（`"router-link-exact-active"`）
- `trailingSlash`：用于在 `href` 中添加或移除尾部斜杠的选项。如果未设置，或不符合有效值 `append` 或 `remove`，则会被忽略。
- `prefetch`：是否默认预取链接。
- `prefetchOn`：更细粒度地控制默认应用哪些预取策略。
