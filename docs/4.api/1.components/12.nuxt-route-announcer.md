...
在你的 [`app.vue`](/docs/3.x/directory-structure/app) 或 [`layouts/`](/docs/3.x/directory-structure/layouts) 中添加 `<NuxtRouteAnnouncer/>`，通过通知辅助技术页面标题变化来提升无障碍体验。这确保依赖屏幕阅读器的用户能够听到导航更改的提示。
...
::callout
你可以通过[ `useRouteAnnouncer` 组合式函数](/docs/3.x/api/composables/use-route-announcer)访问底层播报实例，从而设置自定义播报信息。
::