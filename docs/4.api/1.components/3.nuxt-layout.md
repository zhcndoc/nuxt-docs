...
## 从页面元信息（Page Meta）获取的 Layout Props

当使用 [`definePageMeta`](/docs/4.x/api/utils/define-page-meta) 并以 `layout` 的对象语法传参时，props 会自动传递给布局组件。布局可以使用 `defineProps` 接收它们：
