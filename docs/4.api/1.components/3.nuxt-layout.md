...
## 从页面元信息传递 Layout Props

当使用 [`definePageMeta`](/docs/4.x/api/utils/define-page-meta) 中的对象语法为 `layout` 配置时，props 会自动传递给布局组件。布局可以通过 `defineProps` 接收它们：
