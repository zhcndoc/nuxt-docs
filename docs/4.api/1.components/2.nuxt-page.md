...
`<NuxtPage>` 是 Nuxt 内置的组件。它允许你显示位于 [`pages/`](/docs/3.x/directory-structure/pages) 目录中的顶级或嵌套页面。
...
另外，你也可以通过位于 `/pages` 目录中 Vue 组件 `<script>` 部分的 [`definePageMeta`](/docs/3.x/api/utils/define-page-meta) 传递 `pageKey` 作为 `key` 值。
