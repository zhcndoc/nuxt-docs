...
我们已经准备了一个 [专门指南](/docs/4.x/guide/modules)，用于使用 `@nuxt/kit` 编写面向 Nuxt 3 的模块。当前最佳的迁移路径是遵循该指南并重写你的模块。本指南的其余部分包含一些准备步骤，适用于那些希望避免全面重写但又想使模块与 Nuxt 3 兼容的情况。
...
::tip
在 [模块作者指南](/docs/4.x/guide/modules) 中继续阅读有关 Nuxt 3 模块的内容。
::
