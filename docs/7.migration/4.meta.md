...
1. 通过你的 `nuxt.config`。
2. 通过 [`useHead`](/docs/3.x/api/composables/use-head) [组合式函数](/docs/3.x/getting-started/seo-meta)
3. 通过 [全局元组件](/docs/3.x/getting-started/seo-meta)
...
2. 如果你需要通过 `head` 访问组件状态，应迁移使用 [`useHead`](/docs/3.x/api/composables/use-head)，也可以考虑使用内置的元组件。
