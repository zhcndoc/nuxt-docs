...
考虑创建一个 `app.vue` 文件，并将任何需要在应用顶层只运行一次的逻辑放入其中。你可以在 [此处查看示例](/docs/4.x/guide/directory-structure/app)。

## 布局

如果你的应用在多个页面中使用布局，仅需做很小的更改。

在 Nuxt 2 中，布局中使用 `<Nuxt>` 组件来渲染当前页面。在 Nuxt 3 中，布局改为使用插槽，因此你需要将该组件替换为 `<slot />`。这也允许使用具名插槽和作用域插槽的高级用例。 [阅读有关布局的更多内容](/docs/4.x/guide/directory-structure/app/layouts)。
...
2. 使用 [`definePageMeta`](/docs/4.x/api/utils/define-page-meta) 来选择页面使用的布局。
    ...
    ```
3. 将 `~/layouts/_error.vue` 移动到 `~/error.vue`。参见 [错误处理文档](/docs/4.x/getting-started/error-handling)。如果你想确保该页面使用某个布局，可以在 `error.vue` 中直接使用 [`<NuxtLayout>`](/docs/4.x/guide/directory-structure/app/layouts)：
    ```vue [error.vue]
    <template>
      <div>
...
当源目录中存在 [`app/pages/`](/docs/4.x/guide/directory-structure/app/pages) 目录时，Nuxt 3 会提供一个可选的 `vue-router` 集成。如果你只有一个页面，考虑将其移动到 `app.vue` 以获得更轻量的构建。
