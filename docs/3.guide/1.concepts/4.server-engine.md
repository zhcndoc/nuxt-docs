...
服务器 [API 端点](/docs/4.x/directory-structure/server#server-routes) 和 [中间件](/docs/4.x/directory-structure/server#server-middleware) 由 Nitro 添加，底层使用了 [h3](https://github.com/h3js/h3)。
...
Nuxt 在运行 `nuxt build` 时，会生成该构建文件到 [`.output`](/docs/4.x/directory-structure/output) 目录。
