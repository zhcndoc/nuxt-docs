...
The `200.html` 文件用于需要为客户端路由提供成功状态回退的静态主机。The `404.html` 文件用于在保留 404 状态的同时提供未找到页面的静态主机。你应使用哪个文件取决于你的托管提供商的回退或重写设置。
...
::note
使用 `isr` 或 `swr` 路由时，还会在 HTML 文件旁边生成 `_payload.json` 文件。客户端导航时会加载这些缓存的负载数据，而不是重新抓取数据。动态路由（例如 `pages/[...slug].vue`）可以使用通配符配置：`'/**': { isr: true }`。
::
