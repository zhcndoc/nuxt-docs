...
`nuxt generate` 和 `nuxt build --prerender` 会将它们写入 `.output/public/`。仅执行不带预渲染的 `nuxt build` 不会生成这些文件。如果使用混合路由规则，请通过路由规则添加这些回退文件，或者在需要它们时运行预渲染构建。将你的托管服务指向提供商所期望的那个文件。
...
由于每个缺失路径都提供同一个文件，错误页面不能依赖请求：`useRoute()`、`useRequestURL()` 以及使用当前路径获取的数据，会在预渲染 HTML 中保留构建时的值，并在水合时得到修正。将特定于请求的标记包裹在 [`<ClientOnly>`](/docs/4.x/api/components/client-only) 中，并使用 `import.meta.prerender` 跳过特定于请求的数据获取：
...
- `ssr: boolean`{lang=ts} - 禁用应用部分区域的 HTML 服务器端渲染，使其仅在浏览器中渲染，并使用 `ssr: false`
- `cors: boolean`{lang=ts} - 使用 `cors: true` 自动添加 cors 头；你可以通过使用 `headers` 覆盖来定制输出
- `headers: object`{lang=ts} - 为网站的部分区域添加指定的头；例如你的资源
- `swr: number | boolean`{lang=ts} - 为服务器响应添加缓存头，并在服务器或反向代理上根据可配置的 TTL（生存时间）进行缓存。Nitro 的 `node-server` 预设能够缓存完整响应。当 TTL 过期后，将发送缓存的响应，同时在后台重新生成页面。如果使用 `true`，则会添加 `stale-while-revalidate` 头，但不设置 MaxAge。
- `isr: number | boolean`{lang=ts} - 行为与 `swr` 相同，但对于支持此功能的平台，我们能够将响应添加到 CDN 缓存中（目前支持 Netlify 或 Vercel）。如果使用 `true`，内容将保留在 CDN 中，直到下一次部署。
- `prerender: boolean`{lang=ts} - 在构建时预渲染路由，并将其作为静态资源包含在构建结果中
- `noScripts: boolean`{lang=ts} - 禁用网站部分区域的 Nuxt 脚本和 JS 资源提示。更多信息请参阅 [`noScripts`](/docs/4.x/guide/going-further/features#noscripts)。
- `appMiddleware: string | string[] | Record<string, boolean>`{lang=ts} - 允许你定义在 Vue 应用部分中页面路径应运行或不应运行的中间件（即不包括 Nitro 路由）

::note
使用 `isr` 或 `swr` 的路由还会在 HTML 旁生成 `_payload.json` 文件。客户端导航会加载这些缓存的 payload，而不是重新获取数据。更多关于 [payload 提取](/docs/4.x/getting-started/prerendering#payload-extraction) 的内容。
::

### 使用 `ssr: false` 时的服务器 Bundle 大小

受 `ssr: false` 覆盖的路由只会在浏览器中渲染，因此 Nuxt 会从服务器 Bundle 中排除其页面组件。只要覆盖每个可到达该页面的路径的规则都能在构建时解析，就会应用此规则，包括 `/products/**` 规则下的 `pages/products/[id].vue` 等动态路由。

当任何可到达该页面的路径仍可能在服务器上渲染时，页面会保留在服务器 Bundle 中：

- 更具体的规则会在仅客户端渲染的规则下方某处重新启用 SSR（`'/admin/**': { ssr: false }`{lang=ts} 与 `'/admin/report': { ssr: true }`{lang=ts}）
