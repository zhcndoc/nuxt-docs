...
:read-more{title="SPA 加载模板" to="/docs/4.x/api/configuration/nuxt-config#spaloadingtemplate"}
::
...
静态主机需要一个用于客户端路由和缺失路径的 HTML 外壳。Nuxt 为此输出两个 SPA 回退文件：

- **`200.html`**：当你希望客户端路由器处理该 URL 时，对不匹配的路径使用此文件。
- **`404.html`**：当主机应保持 404 状态但仍要加载你的应用时，使用此文件。

`nuxt generate` 和 `nuxt build --prerender` 会将这些文件写入 `.output/public/`。不带预渲染的普通 `nuxt build` 不会生成这些文件。使用混合路由规则时，如果你需要这些回退文件，请通过路由规则添加它们，或运行预渲染构建。请将你的主机指向提供商所期望的文件。

#### 在服务器端渲染错误页面

默认情况下，`404.html` 是一个空外壳，因此你的 `error.vue`（以及它的布局和数据）只有在客户端应用启动后才会显示。你可以改为对其进行预渲染：
...
然后，Nuxt 会在构建时使用一个模拟的 404 错误渲染 `error.vue`，并将结果写入 `404.html`。无论主机从哪个 URL 提供该文件，它都会在原位置进行水合。如果你的主机能够提供这些页面，请传入一个介于 400 和 599 之间的状态码数组（`[404, 500]`）来生成其他页面。

由于每个缺失路径都提供同一个文件，错误页面不能依赖请求：`useRoute()`、`useRequestURL()` 以及使用当前路径获取的数据会在预渲染 HTML 中保留构建时的值，并在水合时得到修正。将特定于请求的标记包裹在 [`<ClientOnly>`](/docs/4.x/api/components/client-only) 中，并使用 `import.meta.prerender` 跳过特定于请求的数据获取：
...
`import.meta.prerender` 仅在页面生成期间为 `true`，因此当应用使用服务器部署时，同一个 `error.vue` 仍会在服务器端渲染其数据。
...
- `ssr: boolean`{lang=ts} - 禁用应用部分内容的服务端 HTML 渲染，并使其仅在浏览器中渲染，设置为 `ssr: false`
- `cors: boolean`{lang=ts} - 使用 `cors: true` 自动添加 cors 头部——你可以通过覆盖 `headers` 来自定义输出
- `headers: object`{lang=ts} - 为站点的特定部分添加特定头部，例如你的资源文件
- `swr: number | boolean`{lang=ts} - 向服务器响应添加缓存头，并将其缓存在服务器或反向代理中，TTL（存活时间）可配置。Nitro 的 `node-server` 预设能够缓存完整响应。当 TTL 过期时，会在后台重新生成页面的同时发送缓存响应。如果使用 `true`，会添加一个 `stale-while-revalidate` 头，但不带 MaxAge。
- `isr: number | boolean`{lang=ts} - 行为与 `swr` 相同，不同之处在于我们能够在支持该功能的平台上（目前是 Netlify 或 Vercel）将响应加入 CDN 缓存。如果使用 `true`，内容会保留在 CDN 中直到下次部署。
- `prerender: boolean`{lang=ts} - 在构建时预渲染路由，并将它们作为静态资源包含在你的构建中
- `noScripts: boolean`{lang=ts} - 禁用渲染 Nuxt 脚本和 JS 资源提示。阅读更多关于 [`noScripts`](/docs/4.x/guide/going-further/features#noscripts) 的内容。
- `appMiddleware: string | string[] | Record<string, boolean>`{lang=ts} - 允许你定义中间件，这些中间件应或不应在应用的 Vue 部分中的页面路径上运行（也就是，不是你的 Nitro 路由）

::note
使用 `isr` 或 `swr` 的路由也会在 HTML 旁边生成 `_payload.json` 文件。客户端导航会加载这些缓存的 payload，而不是重新请求数据。阅读更多关于[负载提取](/docs/4.x/getting-started/prerendering#payload-extraction)的内容。
::

### 使用 `ssr: false` 时的服务器包大小

由 `ssr: false` 覆盖的路由只会在浏览器中渲染，因此 Nuxt 会将其页面组件排除在服务器包之外。当所有能够到达该页面的路径所对应的规则都可以在构建时解析时，均适用此规则，包括 `/products/**` 规则下的 `pages/products/[id].vue` 等动态路由。

当任何能够到达该页面的路径仍可能在服务器上渲染时，页面会保留在服务器包中：

- 更具体的规则会在客户端专属规则的下方某处重新启用 SSR（`'/admin/**': { ssr: false }`{lang=ts}，并配有 `'/admin/report': { ssr: true }`{lang=ts}）
- 页面具有一个别名，或具有使用绝对路径声明的子页面，并且该别名或子页面位于客户端专属区域之外
- 页面是一个父级外壳，用于渲染仍在服务器上渲染的子页面

这只是构建时优化；它不会改变服务器发送到浏览器的内容。

在可能的情况下，路由规则会自动应用到部署平台的原生规则中，以实现最佳性能（目前支持 Netlify 和 Vercel）。
