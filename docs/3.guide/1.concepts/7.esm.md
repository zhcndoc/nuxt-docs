...
::note
`module` 字段是打包器（例如 webpack 和 Rollup）约定使用的字段，Node.js 本身并不识别它。Node.js 在模块解析时只会使用 [`exports`](https://nodejs.org/api/packages.html#exports) 和 [`main`](https://nodejs.org/api/packages.html#main) 字段。
::
...
当你使用 `import` 而不是 `require` 导入一个模块时，Node.js 的解析方式会不同。例如，当你导入 `sample-library` 时，Node.js 会查找该库的 `package.json` 中的 `exports` 条目，如果未定义则回退到 `main` 条目。
...
对于更复杂的情况以及更高的安全性，我们推荐使用 [mlly](https://github.com/unjs/mlly)，它可以保留具名导出。

```js
import { interopDefault } from 'mlly'

// 假设其结构为 { default: { foo: 'bar' }, baz: 'qux' }
...
const someFile = resolveModulePath('my-lib', { from: import.meta.url })
```
