...
::warning
环境变量值会使用 [`destr`](https://github.com/unjs/destr) 自动转换为其 JavaScript 类型。例如，`NUXT_MY_VAR=4848e0` 会变成数字 `4848`。如果要保持值为字符串，环境变量值本身必须包含字面量双引号：在 `.env` 文件中，写成 `NUXT_MY_VAR='"4848e0"'`；当直接设置变量时（在 shell、Dockerfile 或托管面板中），请确保引号是值的一部分，而不是被 shell 去掉（例如 `NUXT_MY_VAR='"4848e0"' node .output/server/index.mjs`）。
::
...
export default defineNuxtConfig({
  runtimeConfig: {
    apiSecret: '', // 可被 NUXT_API_SECRET 环境变量覆盖
    public: {
      apiBase: '', // 可被 NUXT_PUBLIC_API_BASE 环境变量覆盖
    },
  },
})
```
