...
目前，Nuxt 上下文仅在 [插件](/docs/3.x/directory-structure/plugins)、[Nuxt 钩子](/docs/3.x/guide/going-further/hooks)、[Nuxt 中间件](/docs/3.x/directory-structure/middleware)（如果通过 `defineNuxtRouteMiddleware` 包装）以及 [setup 函数](https://vuejs.org/api/composition-api-setup)（页面和组件中）可以访问。
