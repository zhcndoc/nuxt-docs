...
当扩展一个层时，你可能想禁用该层包含的某些模块。你可以在 Nuxt 配置中将模块的配置键设置为 `false` 来实现。
...
::note
配置键由各模块定义。常见示例如 `image` 对应 `@nuxt/image`，`pinia` 对应 `@pinia/nuxt`，`content` 对应 `@nuxt/content`。请查看各模块文档获取具体配置键。
::
...
::tip
你也可以用此方法禁用你自己项目中的模块，而不仅仅是层中的。将模块配置键设为 `false` 会阻止其 setup 函数运行，同时仍会生成模块的类型。
::

## 多层支持的 Nuxt 模块

你可以使用 Nuxt Kit 的 [`getLayerDirectories`](/docs/3.x/api/kit/layers#getlayerdirectories) 工具，为你的模块支持自定义多层处理。
