...
不含脚本的页面会通过整页加载进行导航。为了保持导航速度，Nuxt 会在这些页面上输出一个 [推测规则](https://developer.mozilla.org/en-US/docs/Web/API/Speculation_Rules_API) 标签，使支持该功能的浏览器在用户点击链接之前预取并预渲染目标页面。该标签是声明式 JSON，不会执行任何 JavaScript。规则的作用范围限定为你的页面路由（这些路由可以安全地使用 `GET` 请求），而不是所有同源链接，因此指向 `/logout` 等服务器路由的链接不会被推测性地获取。全匹配页面（例如 `pages/[...slug].vue`）会匹配所有路径，从而将作用范围扩展回所有同源链接。

客户端导航*到*被 `noScripts` 路由规则覆盖的路由时，会改为触发完整的文档加载，因此该路由实际上是在不含脚本的情况下提供服务，而不是由客户端路由器渲染。那些在构建时已知其路由受此类规则覆盖的页面，也会完全从客户端 bundle 中排除。

包含脚本的页面会输出其自己的推测规则标签，并将作用范围限定为 `noScripts` 路由模式，因此浏览器可以在导航到这些目标页面并触发整页加载之前预取和预渲染它们。启用视图过渡时，这些页面还会选择加入声明式跨文档视图过渡，以便为导航添加动画效果。
...
参见 [原始的 TypeScript 拉取请求](https://github.com/microsoft/TypeScript/pull/51669)。
