...
Allows JSON payloads to be rendered in a way that supports revivifying complex types.
...
Enable the experimental [`<NuxtClientFallback>`](/docs/4.x/api/components/nuxt-client-fallback) component so content can be rendered on the client when SSR errors occur.
...
Wait one animation frame before navigation, giving the browser a chance to repaint in response to user interaction.

When navigating to a prerendered route, this can reduce INP.
...
Use head optimizations:

- Add the capo.js head plugin to render tags in the head more efficiently.
- Use the hash hydration plugin to reduce initial hydration.
