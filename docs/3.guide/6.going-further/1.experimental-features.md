...
不带有 `Accept: text/html` 标头的请求（API 客户端、大多数扫描器）会直接收到 JSON 错误，而完全不会加载应用。HTML 请求仍会进行服务端渲染以生成错误页面。覆盖该路径的 [`cache` 路由规则](/docs/4.x/guide/concepts/rendering#hybrid-rendering)也会在 GET/HEAD 未命中时通过 `Cache-Control` 标头公布其 `maxAge`，从而让 CDN 吸收对未知路径的重复请求。
...
- 在插件中通过 `router.addRoute()` 动态添加的页面。启用此选项后，从 [`useRouter`](/docs/4.x/api/composables/use-router) 返回的路由器类型中会移除 `addRoute`，以便尽早暴露此问题。
- 将未知路径重定向到现有页面的路由中间件
...
启用实验性的 [`<NuxtClientFallback>`](/docs/4.x/api/components/nuxt-client-fallback) 组件，这样当发生 SSR 错误时，内容就可以在客户端渲染。
...
你也可以传入一个对象来配置[视图过渡类型](/docs/4.x/getting-started/transitions#view-transition-types)，从而根据导航类型使用不同的 CSS 动画：
...
使用 [`<NuxtIsland>`](/docs/4.x/api/components/nuxt-island) 和 `.island.vue` 文件启用实验性的组件岛支持。
...
::note
在非 SFC 组件（例如 [`<NuxtLink>`](/docs/4.x/api/components/nuxt-link)）上跳过 `nuxt-client`。请参阅[使用 `nuxt-client` 进行选择性水合](/docs/4.x/guide/concepts/server-components#selective-hydration-with-nuxt-client)。
::
...
`useState.resetOnClear` 选项用于控制 [`clearNuxtState`](/docs/4.x/api/utils/clear-nuxt-state) 是否将状态重置为其初始值（由 [`useState`](/docs/4.x/api/composables/use-state) 的 `init` 函数提供），而不是将状态设置为 `undefined`。在 `compatibilityVersion: 5` 下，默认值为 `true`。
...
当 `<NuxtLink>` 被预取且目标路由启用了 [负载提取](#payloadextraction)（预渲染和缓存路由的默认设置）时，会将目标通过 [`useHead`](/docs/4.x/api/composables/use-head)（或通过 [`@nuxt/image`](https://image.nuxt.com) 的 `<NuxtImg preload>` 等模块）设置的任何 `<link rel="preload">` 提示转发到当前文档中。
...
当 `<NuxtLink>` 被预取，并且目标路由启用了 [payload extraction](#payloadextraction)（这是预渲染和已缓存路由的默认行为）时，会将目标通过 [`useHead`](/docs/4.x/api/composables/use-head)（或通过诸如 [`@nuxt/image`](https://image.nuxt.com) 的 `<NuxtImg preload>` 之类的模块）设置的任何 `<link rel="preload">` 提示转发到当前文档中。
...
默认对在使用 `hydrate-never` 延迟水合的组件中调用的 `useAsyncData` 和 `useFetch` 应用 [`serialize: false`](/docs/4.x/api/composables/use-async-data#params)，使其数据不会进入 `__NUXT_DATA__` 负载。由于这些组件永远不会在客户端进行水合，因此水合不需要这些数据。
...
- `routeRules` 为该路由设置 `noScripts`、`cache`、`isr`、`swr`、`redirect` 或 `streaming: false`
