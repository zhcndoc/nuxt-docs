...
Whether to run `useFetch` when the key changes, even if it is set to `immediate: false` and has not yet been triggered.

If `immediate: true` or already triggered, `useFetch` and `useAsyncData` will always run when the key changes.
...
Use the app manifest to follow routing rules on the client side.
...
Use [`defineRouteRules`](/docs/4.x/api/utils/define-route-rules) to define route rules at the page level.
...
Create matching route rules based on the page's `path`.
...
当预取 `<NuxtLink>` 且目标路由启用了 [payload extraction](#payloadextraction)（预渲染和缓存路由的默认设置）时，会将目标通过 [`useHead`](/docs/4.x/api/composables/use-head)（或通过 [`@nuxt/image`](https://image.nuxt.com) 的 `<NuxtImg preload>` 等模块）设置的任何 `<link rel="preload">` 提示转发到当前文档中。
