...
- `'client'` - 对于初始服务器渲染，payload 内联在 HTML 中；对于客户端导航，则提取到 `_payload.json` 文件中。这避免了初始加载时的单独网络请求，同时仍然实现高效的客户端导航。
- `true` - 对于初始服务器渲染和客户端导航，payload 都提取到单独的 `_payload.json` 文件中。
- `false` - 完全禁用 payload 提取。payload 始终内联在 HTML 中，不生成 `_payload.json` 文件。
