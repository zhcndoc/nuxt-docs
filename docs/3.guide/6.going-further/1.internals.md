...
我们可以使用 [Nuxt 模块](/docs/4.x/guide/modules) 来扩展 Nuxt 接口并在构建流程的不同阶段挂载钩子。
...
可以使用 `nuxt.config` 和 [Nuxt 模块](/docs/4.x/guide/modules) 来扩展构建上下文，使用 [Nuxt 插件](/docs/4.x/directory-structure/app/plugins) 来扩展运行时。

当为生产构建应用时，`nuxt build` 会在 `.output` 目录下生成一个独立的构建，该构建与 `nuxt.config` 和 [Nuxt 模块](/docs/4.x/guide/modules) 无关。
