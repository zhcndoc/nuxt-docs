...
在 Nuxt 中，你的路由由 [app/pages 目录](/docs/4.x/directory-structure/app/pages) 内文件的结构定义。然而，由于其底层使用 [vue-router](https://router.vuejs.org)，Nuxt 为你提供了几种在项目中添加自定义路由的方法。

### 路由配置

使用 [路由选项](/docs/4.x/guide/recipes/custom-routing#router-options)，你可以可选地通过一个接收已扫描路由并返回自定义路由的函数来覆盖或扩展路由。
...
除了为 [`vue-router`](https://router.vuejs.org/api/interfaces/routeroptions) 自定义选项之外，Nuxt 还提供了 [额外选项](/docs/4.x/api/nuxt-config#router) 来自定义路由器。
...
::note
使用 `future.compatibilityVersion: 5` 时，路由默认区分大小写，以与 Nitro 保持一致。将 `router.options.sensitive` 设为 `false` 可选择退出。
::
