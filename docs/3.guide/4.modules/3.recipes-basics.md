...
::note
请注意，所有通常放置在你的 `app/` 文件夹中的组件、页面、组合函数和其他文件都需要放在 `runtime/app/` 中。这将确保它们能够被正确进行类型检查。
::
...
::warning
键注入插件会验证每个函数调用的精确解析导入来源。它不支持通过中转导出（桶文件）调用。函数必须从 `source` 属性指定的确切源文件导出。
...
::warning
函数调用必须是静态可分析的。编译器无法为动态或间接调用注入键。
