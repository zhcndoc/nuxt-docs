...
::warning
当在路由文件中使用 **仅限服务器的类型** 时，可能会导致问题。例如，如果某个模块使用 [`addServerTemplate`](/docs/api/kit/templates#addservertemplate) 创建了一个仅限服务器的虚拟文件，并在 `tsconfig.server.json` 中声明了该文件的类型，那么这些类型声明只会在服务器上下文中可用。当 app 上下文对服务器路由进行类型检查时，它不会识别这些仅限服务器的类型，从而报告错误。为了解决这个问题，你不得不在 app 上下文中也声明这些类型。
::