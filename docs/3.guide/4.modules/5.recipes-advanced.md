...
::warning
这在路由文件中使用**仅服务器端类型**时可能会导致问题。例如，如果某个模块使用 [`addServerTemplate`](/docs/4.x/api/kit/templates#addservertemplate) 创建了一个仅服务器端虚拟文件，并且你在 `tsconfig.server.json` 中为其声明了类型，那么这些类型声明只会在服务器上下文中可用。当 app 上下文对你的服务器路由进行类型检查时，它不会识别这些仅服务器端类型，并会报告错误。要解决此问题，你遗憾地还需要在 app 上下文中声明这些类型。
::
