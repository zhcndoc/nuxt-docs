...
Nuxt 的[配置](/docs/3.x/api/nuxt-config)和[钩子](/docs/3.x/guide/going-further/hooks)系统使得定制 Nuxt 的每个方面并添加任何集成都成为可能（Vue 插件、CMS、服务器路由、组件、日志记录等）。
...
- "本地" 模块：存在于 Nuxt 项目内部，要么内联于 Nuxt 配置中（[内联模块](/docs/3.x/api/nuxt-config#modules)），要么作为 [`modules` 目录](/docs/3.x/directory-structure/modules) 的一部分。
...
你可以使用 [Nuxt Kit](/docs/3.x/guide/going-further/kit) 提供的高级辅助 `defineNuxtModule` 获得类型提示支持。
...
:br :br
实际上，出于性能考虑，自动导入不会对 `node_modules`（即发布模块的所在位置）内的文件启用。
::
...
[Nuxt Kit](/docs/3.x/guide/going-further/kit) 提供了组合式工具，辅助模块与 Nuxt 应用交互。建议你在可能的情况下使用 Nuxt Kit 工具，确保模块兼容性和代码可读性。

:read-more{to="/docs/guide/going-further/kit"}

#### `@nuxt/test-utils`

[Nuxt 测试工具](/docs/3.x/getting-started/testing)是一个集合，帮助在模块测试中搭建和运行 Nuxt 应用。
...
模块不在应用运行时中，因此其选项也不会被包含。但通常你可能需要在运行时代码中访问模块选项。建议通过 Nuxt 的 [`runtimeConfig`](/docs/3.x/api/nuxt-config#runtimeconfig) 暴露必要的配置。
...
更高级地，可通过 [Nitro](/docs/3.x/guide/concepts/server-engine) 的 `publicAssets` 选项暴露一整文件夹的资源：
...
::callout{type="info"}
`moduleDependencies` 选项取代已废弃的 `installModule` 函数，确保正确的安装顺序与配置合并。
::

#### 使用钩子

[生命周期钩子](/docs/3.x/guide/going-further/hooks)允许你扩展 Nuxt 的各个方面。模块可通过 `hooks` 对象声明或编程式调用 `nuxt.hook` 注册钩子。
...
[Nuxt 测试工具](/docs/3.x/getting-started/testing) 是模块端到端测试的首选库。使用流程：
...
此模式避免每次构建都执行耗时操作，提升开发体验。详情请参阅 [生命周期钩子文档](/docs/3.x/api/kit/modules#using-lifecycle-hooks-for-module-installation-and-upgrade)。
...
Nuxt 依赖原生 ESM。请查阅[原生 ES 模块](/docs/3.x/guide/concepts/esm)获取更多信息。
