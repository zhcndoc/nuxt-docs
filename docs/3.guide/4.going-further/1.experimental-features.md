...
Nuxt 内部使用 `@nuxt/schema` 来定义这些实验性功能。您可以参考[API 文档](/docs/3.x/api/configuration/nuxt-config#experimental) 或 [源代码](https://github.com/nuxt/nuxt/blob/main/packages/schema/src/config/experimental.ts) 获取更多信息。
...
::important
启用此功能会改变 Vite 插件的注册和配置方式。详见 [Vite 环境 API 迁移指南](/docs/4.x/getting-started/upgrade#migration-to-vite-environment-api) 了解插件更新细节。
::
