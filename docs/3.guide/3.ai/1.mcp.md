...
- **`list-documentation-pages`**：列出所有可用的 Nuxt 文档页面及其类别和基本信息。支持版本筛选（3.x、4.x、5.x 或全部）
- **`get-documentation-page`**：根据路径获取文档页面内容和详细信息
- **`get-getting-started-guide`**：获取特定 Nuxt 版本的入门指南

### 博客

- **`list-blog-posts`**：列出所有 Nuxt 博客文章及其元数据，包括日期、类别和标签
- **`get-blog-post`**：根据路径获取博客文章内容和详细信息

### 部署

- **`list-deploy-providers`**：列出所有适用于 Nuxt 应用的部署提供商和托管平台
- **`get-deploy-provider`**：根据路径获取部署提供商的详细信息和说明

### 模块

- **`list-modules`**：列出所有可用的 Nuxt 模块，可按下载量、星标数或日期进行筛选和排序
- **`get-module`**：获取特定模块的完整详细信息，包括 README、兼容性、维护者和统计数据

### 更新日志

- **`get-changelog`**：获取 Nuxt 核心和官方模块的最新版本发布信息

## 限制可用工具

设置可选的 `X-MCP-Tools` HTTP 标头，其值为以逗号分隔的工具名称列表，即可仅公开助手需要的工具。这样可以减少发送给助手的工具上下文。

使用服务器返回的 MCP 工具名称对应的确切短横线命名格式，例如：
...
不设置标头会公开所有工具，空标头不会公开任何工具，未知工具名称会返回配置错误。

## 提示

Nuxt MCP 服务器提供了针对常见工作流的引导式提示：

- **`find-documentation-for-topic`**：查找特定主题或功能对应的最佳 Nuxt 文档
- **`deployment-guide`**：获取特定托管提供商的部署说明
- **`migration-help`**：获取 Nuxt 版本迁移方面的帮助
