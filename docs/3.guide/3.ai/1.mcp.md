...
- **`get-getting-started-guide`**：获取指定 Nuxt 版本的入门指南
...
- **`list-modules`**：列出所有可用的 Nuxt 模块，可选择按下载量、星标数或日期进行筛选和排序
...
设置可选的 `X-MCP-Tools` HTTP 标头，其值为以逗号分隔的工具名称列表，即可只公开助手所需的工具。这会减少发送给助手的工具上下文。

使用服务器返回的确切 kebab-case MCP 工具名称，例如：
...
- **`find-documentation-for-topic`**：查找有关特定主题或功能的最佳 Nuxt 文档
- **`deployment-guide`**：获取针对特定托管提供商的部署说明
- **`migration-help`**：获取 Nuxt 版本之间迁移方面的帮助
...
3. 在配置表单中填写：
   ...
   - **HTTP 头**：留空
   - **工具**：`*`
4. 按 <kbd>Ctrl</kbd>+<kbd>S</kbd> 保存。无需重启 CLI，MCP 服务器会立即可用。
