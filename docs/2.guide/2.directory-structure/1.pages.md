...
// https://nuxt.zhcndoc.com/docs/examples/advanced/jsx
...
与 `defineEmits` 或 `defineProps` 类似（参见 [Vue 文档](https://vue.zhcndoc.com/api/sfc-script-setup.html#defineprops-defineemits)），`definePageMeta` 是一个 **编译宏**。它将在编译时被移除，因此您无法在组件内引用它。相反，传递给它的元数据将被提升到组件外部。
...
如果你在 `definePageMeta` 中设置了 `keepalive: true`，Nuxt 将自动将你的页面包裹在 [Vue `<KeepAlive>` 组件](https://vue.zhcndoc.com/guide/built-ins/keep-alive.html#keepalive)中。这可能在你想要在路由变化时保留页面状态时有用。

当你的目标是保留父路由的状态时，可以使用这个语法：`<NuxtPage keepalive />`。你也可以设置 `<KeepAlive>` 的 props（参见[这里](https://vue.zhcndoc.com/api/built-in-components.html#keepalive)或阅读[更多关于如何过渡](https://vue.zhcndoc.com/guide/built-ins/keep-alive.html)。
...
你可以为包裹页面的 `<transition>` 组件定义转换属性，或通过传递 `false` 来禁用该页面的 `<transition>` 包装器。你可以在这里看到可以传递的选项[](https://vue.zhcndoc.com/api/built-in-components.html#transition)或阅读[更多关于转换的工作方式](https://vue.zhcndoc.com/guide/built-ins/transition.html#transition)。
