...
::note
为了减少你的应用程序的捆绑包大小，这个目录是**可选的**，这意味着如果只使用 `~/pages/app.vue`，`vue-router` 将不会被包含。要强制使用页面系统，请在 `nuxt.config` 中设置 `pages: true`，或者有一个 `app/router.options.ts`。
::
...
Nuxt 3 允许通过 `navigateTo()` 实用工具方法进行程序化导航。使用这个实用工具方法，你将能够以编程方式在你的应用中导航用户。这很适合从用户那里获取输入并动态地在整个应用中引导他们。在这个例子中，我们有一个简单的叫做 `navigate()` 的方法，在用户提交搜索表单时被调用。
