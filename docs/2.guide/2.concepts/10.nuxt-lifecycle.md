...
Nuxt 由现代服务器引擎 [Nitro](https://nitro.zhcndoc.com/) 提供支持。
...
- 位于 `app/plugins/` 目录的自定义插件，包括无后缀的（例如 `myPlugin.ts`）以及带 `.server` 后缀的（例如 `myServerPlugin.server.ts`）。

插件按特定顺序执行，且可能相互依赖。有关执行顺序和并行性的更多详细信息，请参阅[插件文档](/docs/4.x/guide/directory-structure/plugins)。

::callout{icon="i-lucide-lightbulb"}
在此步骤之后，Nuxt 会调用 [`app:created`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) 钩子，可用于执行额外逻辑。
::
...
有关每种类型的详细信息和示例，请参阅[中间件文档](/docs/4.x/guide/directory-structure/app/middleware)。
...
Nuxt 在此步骤中渲染页面及其组件，并使用 `useFetch` 和 `useAsyncData` 获取任何所需的数据。由于服务器上不存在动态更新且不会发生 DOM 操作，Vue 生命周期钩子例如 `onBeforeMount`、`onMounted` 及随后的钩子在 SSR 期间是**不会**被执行的。

默认情况下，为了更好的性能，Vue 在 SSR 期间会暂停依赖追踪。

::callout{icon="i-lucide-lightbulb"}
服务器端没有响应性，因为 Vue SSR 将应用自顶向下渲染为静态 HTML，一旦渲染完成就无法回头修改已经渲染的内容。
::

::important
应避免在 `<script setup>` 的根作用域中放置需要清理的副作用代码。一个示例是使用 `setInterval` 设置定时器。在仅客户端的代码中，我们可以在 `onBeforeUnmount` 或 `onUnmounted` 中销毁定时器。然而，因为在 SSR 期间卸载钩子永远不会被调用，这些定时器将永久存在。为避免此类问题，请将副作用代码移动到 `onMounted` 中。
::
...
::callout{icon="i-lucide-lightbulb"}
在将 Vue 应用渲染为 HTML 之后，Nuxt 会调用 [`app:rendered`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) 钩子。
::

::callout{icon="i-lucide-lightbulb"}
在最终确定并发送 HTML 之前，Nitro 会调用 [`render:html`](/docs/4.x/api/advanced/hooks#nitro-app-hooks-runtime-server-side) 钩子。该钩子允许你操纵生成的 HTML，例如注入额外脚本或修改 meta 标签。
::
...
调用 `app.mount('#__nuxt')` 会将 Vue 应用挂载到 DOM。如果应用使用了 SSR 或 SSG 模式，Vue 会执行水合步骤以使客户端应用具有交互性。在水合期间，Vue 会重建应用（不包括[服务器组件](/docs/4.x/guide/directory-structure/app/components#server-components)），将每个组件与其对应的 DOM 节点匹配，并附加 DOM 事件监听器。
...
::callout{icon="i-lucide-lightbulb"}
在挂载 Vue 应用之前，Nuxt 会调用 [`app:beforeMount`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) 钩子。
::

::callout{icon="i-lucide-lightbulb"}
在挂载 Vue 应用之后，Nuxt 会调用 [`app:mounted`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) 钩子。
::

### 步骤 5：Vue 生命周期

与服务器不同，浏览器会完整执行 [Vue 生命周期](https://vue.zhcndoc.com/guide/essentials/lifecycle)。
