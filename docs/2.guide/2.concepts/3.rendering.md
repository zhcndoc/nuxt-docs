...
[中间件](/docs/4.x/guide/directory-structure/app/middleware) 和 [页面](/docs/4.x/guide/directory-structure/app/pages) 会在服务器上运行，并在水合期间在客户端运行。[插件](/docs/4.x/guide/directory-structure/plugins) 可以在服务器或客户端或两者上运行。[组件](/docs/4.x/guide/directory-structure/app/components) 也可以被强制仅在客户端运行。[组合式函数（Composables）](/docs/4.x/guide/directory-structure/app/composables) 和 [工具函数](/docs/4.x/guide/directory-structure/app/utils) 则根据它们的使用上下文决定运行位置。
...
::tip
关于编写不会导致水合不匹配（hydration mismatch）的 Vue 代码的更多示例，请参阅 [Vue 官方文档](https://vue.zhcndoc.com/guide/scaling-up/ssr.html#hydration-mismatch)。
::
...
当前可以利用 ESR 的平台有：
- 使用 git 集成和 `nuxt build` 命令的 [Cloudflare Pages](https://pages.cloudflare.com)，无需任何配置
- 使用 `nuxt build` 命令并设置环境变量 `NITRO_PRESET=vercel-edge` 的 [Vercel Edge Functions](https://vercel.com/features/edge-functions)
- 使用 `nuxt build` 命令并设置环境变量 `NITRO_PRESET=netlify-edge` 的 [Netlify Edge Functions](https://www.netlify.com/products/#netlify-edge-functions)
...
::card-group
  ...
  ui.icon.base: text-black dark:text-white
  ---
  一个带有用户身份验证、SSR 和 SQLite 的待办事项应用。
  ::
  ::card
  ---
  icon: i-simple-icons-github
