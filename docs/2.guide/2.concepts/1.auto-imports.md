...
得益于其有针对性的目录结构，Nuxt 可以自动导入你的 [`app/components/`](/docs/4.x/guide/directory-structure/app/components)、[`app/composables/`](/docs/4.x/guide/directory-structure/app/composables) 和 [`app/utils/`](/docs/4.x/guide/directory-structure/app/utils)。
...
::note
在 [`server`](/docs/4.x/guide/directory-structure/server) 目录中，Nuxt 会自动导入来自 `server/utils/` 的导出函数和变量。
::
...
- `app/components/` 用于 [Vue 组件](/docs/4.x/guide/directory-structure/app/components)。
- `app/composables/` 用于 [Vue composables](/docs/4.x/guide/directory-structure/app/composables)。
...
这是由于 Vue 对不是模板顶层的 refs 的处理方式所致。你可以在 [Vue 文档](https://vue.zhcndoc.com/guide/essentials/reactivity-fundamentals.html#caveat-when-unwrapping-in-templates) 中阅读更多信息。
::
