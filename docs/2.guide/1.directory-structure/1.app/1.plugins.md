...
你可以在 Nuxt 插件中使用 [composables](/docs/4.x/guide/directory-structure/app/composables) 以及 [utils](/docs/4.x/guide/directory-structure/app/utils)：
...
::important
我们强烈建议使用 [`composables`](/docs/4.x/guide/directory-structure/app/composables) 来替代提供全局辅助方法，以避免污染全局命名空间并保持主包入口的小巧。
::

::warning
**如果你的插件提供了一个 `ref` 或 `computed`，它在组件的 `<template>` 中不会被解包。** :br
这是由于 Vue 对非模板顶层的 ref 的处理方式。你可以在 [Vue 文档](https://vue.zhcndoc.com/guide/essentials/reactivity-fundamentals.html#caveat-when-unwrapping-in-templates) 中阅读更多内容。
::
...
::note
如果你使用 WebStorm，可能需要在该问题解决之前，扩展 `@vue/runtime-core` 的类型声明，详见此问题：[WEB-59818](https://youtrack.jetbrains.com/issue/WEB-59818/VUE-TypeScript-WS-PS-does-not-correctly-display-type-of-globally-injected-properties)。
::
