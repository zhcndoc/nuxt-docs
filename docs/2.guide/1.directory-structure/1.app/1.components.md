...
::note
在内部，这使用了 Vue 的内置 [`hydrateOnVisible` 策略](https://vue.zhcndoc.com/guide/components/async.html#hydrate-on-visible)。
::
...
::note
在内部，这使用了 Vue 的内置 [`hydrateOnIdle` 策略](https://vue.zhcndoc.com/guide/components/async.html#hydrate-on-idle)。
::
...
::note
在内部，这使用了 Vue 的内置 [`hydrateOnInteraction` 策略](https://vue.zhcndoc.com/guide/components/async.html#hydrate-on-interaction)。
::
...
::note
在内部，这使用了 Vue 的内置 [`hydrateOnMediaQuery` 策略](https://vue.zhcndoc.com/guide/components/async.html#hydrate-on-media-query)。
::
...
如果你想从 npm 包自动导入组件，你可以在一个 [本地模块](/docs/4.x/guide/directory-structure/modules) 中使用 [`addComponent`](/docs/4.x/api/kit/components#addcomponent) 来注册它们。
...
服务端组件可以单独使用，也可以与 [客户端组件](/docs/4.x/guide/directory-structure/app/components#paired-with-a-client-component) 配对使用。
...
::note
插槽可以是交互式的，并被包裹在带有 `display: contents;` 的 `<div>` 中。
::
