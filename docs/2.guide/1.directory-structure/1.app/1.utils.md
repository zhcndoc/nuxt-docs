...
::tip
`app/utils/` 的自动导入工作方式和扫描方式与 [`app/composables/`](/docs/4.x/guide/directory-structure/app/composables) 目录完全相同。
::

::important
这些工具只在应用的 Vue 部分可用。 :br
只有 `server/utils` 会在 [`server/`](/docs/4.x/guide/directory-structure/server#server-utilities) 目录中被自动导入。
::