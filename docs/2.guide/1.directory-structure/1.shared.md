...
现在你可以在 Nuxt 应用和 `server/` 目录中使用这些[自动导入](/docs/4.x/guide/directory-structure/shared) 的工具了。
...
::tip
`shared/utils` 和 `shared/types` 的自动导入和扫描方式与 [`app/composables/`](/docs/4.x/guide/directory-structure/app/composables) 和 [`app/utils/`](/docs/4.x/guide/directory-structure/app/utils) 目录的行为相同。
::
