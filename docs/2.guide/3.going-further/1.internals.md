...
由于 [`useNuxtApp`](/docs/4.x/api/composables/use-nuxt-app) 在上下文当前不可用时会抛出异常，如果你的组合式函数并不总是需要 `nuxtApp`，可以使用 [`tryUseNuxtApp`](/docs/4.x/api/composables/use-nuxt-app#tryusenuxtapp) 代替，
它会在不可用时返回 `null` 而不是抛出异常。

我们可以使用 [Nuxt 插件](/docs/4.x/guide/directory-structure/plugins) 来扩展 `nuxtApp` 接口并在不同阶段挂载或访问上下文。
...
可以使用 `nuxt.config` 和 [Nuxt 模块](/docs/4.x/guide/going-further/modules) 来扩展构建上下文，使用 [Nuxt 插件](/docs/4.x/guide/directory-structure/plugins) 来扩展运行时。
