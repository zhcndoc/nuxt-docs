...
这些钩子可供 [Nuxt 模块](/docs/4.x/guide/going-further/modules) 和构建上下文使用。
...
应用钩子主要可被 [Nuxt 插件](/docs/4.x/guide/directory-structure/plugins) 用来挂入渲染生命周期，但也可在 Vue 组合式函数中使用。
...
这些钩子可供 [服务器插件](/docs/4.x/guide/directory-structure/server#server-plugins) 用来挂入 Nitro 的运行时行为。
