...
在内部，Nuxt 使用 `@nuxt/schema` 来定义这些实验性功能。您可以参考 [API 文档](/docs/api/configuration/nuxt-config#experimental) 或 [源代码](https://github.com/nuxt/nuxt/blob/main/packages/schema/src/config/experimental.ts) 获取更多信息。
...
::important
如果您使用 `pnpm` 并且没有设置 `shamefully-hoist=true`，则需要将 `unplugin-vue-router` 作为开发依赖项安装，以使此功能正常工作。
::
...
此外，对 `srcDir` 中的文件的任何更改都会触发 Vue 客户端/服务器捆绑包的重建。Nitro 将始终被重建（尽管是在进行工作以允许 Nitro 宣布其可缓存的工件及其哈希值）。
...
这避免了在 hydrating 仅客户端页面时出现的白色闪烁。

## browserDevtoolsTiming

启用浏览器 DevTools 中 Nuxt 钩子的性能标记。这添加了性能标记，您可以在基于 Chromium 的浏览器的性能选项卡中跟踪，这对于调试和优化性能非常有用。

这在开发模式下默认启用。如果您需要禁用此功能，可以通过以下方式实现：
...
记录模块上下文中的 `nuxt.options` 的变更，帮助调试模块在 Nuxt 初始化阶段所做的配置更改。

当启用 `debug` 模式时，此功能默认开启。如果您需要禁用此功能，可以通过以下方式实现：
...
这启用 `<Lazy>` 组件的水合策略，通过延迟组件的水合直到需要时来提高性能。
...
控制 Nuxt 模板中的导入如何解析。默认情况下，Nuxt 尝试根据添加它们的模块解析模板中的导入。

这在默认情况下启用，因此如果您在某些环境中遇到解析冲突，可以禁用此行为：
...
Nuxt 将自动清除来自 `useAsyncData` 和 `nuxtApp.static.data` 的缓存数据。这有助于防止内存泄漏
并确保在需要时加载新数据，但可以禁用此功能：
