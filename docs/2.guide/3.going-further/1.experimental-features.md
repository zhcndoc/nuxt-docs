...
如果将其设置为 `'automatic-immediate'`，Nuxt 将立即重新加载当前路由，而不是等待导航。这对于不是由导航触发的块错误非常有用，例如，当你的 Nuxt 应用无法加载 [懒组件](/docs/guide/directory-structure/app/components#dynamic-imports) 时。此行为的潜在缺点是可能导致不必要的重新加载，例如，当你的应用不需要导致错误的块时。
...
这允许模块在构建上下文中访问页面元数据中的附加元数据。如果你在模块中使用此功能，建议你 [使用你的键增强 `NuxtPage` 类型](/docs/guide/directory-structure/app/pages#typing-custom-metadata)。
