...
目前，Nuxt 上下文仅可在 [plugins](/docs/4.x/guide/directory-structure/plugins)、[Nuxt hooks](/docs/4.x/guide/going-further/hooks)、[Nuxt middleware](/docs/4.x/guide/directory-structure/app/middleware)（如果用 `defineNuxtRouteMiddleware` 包裹），以及页面和组件中的 [setup 函数](https://vue.zhcndoc.com/api/composition-api-setup.html) 中访问。
