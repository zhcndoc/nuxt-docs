...
- 一个由 [Vitest](https://vitest.dev) 提供支持的测试运行器，可以使用 `npm run test` 或 `npm run test:watch` 运行
...
Nuxt 3 对 TypeScript 有一流的集成，以提供最佳开发体验。

暴露类型和使用 TypeScript 开发模块也能让用户受益，即使他们没有直接使用 TypeScript。

#### 避免使用 CommonJS 语法

Nuxt 3 依赖于原生 ESM。有关更多信息，请阅读 [原生 ES 模块](/docs/guide/concepts/esm)。
...
Nuxt 3、Nuxt Kit 和其他新工具旨在考虑向前和向后的兼容性。
