...
::tip
有关编写 Vue 代码而不产生水合不匹配的更多示例，请参见[Vue 文档](https://vue.zhcndoc.com/guide/scaling-up/ssr.html#hydration-mismatch)。
::
...
Nuxt 服务器将自动注册相应的中间件，并使用[Nitro 缓存层](https://nitro.zhcndoc.com/guide/cache)包装路由。
...
- `noScripts: boolean`{lang=ts} - 禁用该网站部分的 Nuxt 脚本和 JS 资源提示的渲染。
...
边缘侧渲染得益于[Nitro](https://nitro.zhcndoc.com)，这是支持 Nuxt 3 的[服务器引擎](/docs/guide/concepts/server-engine)。它为 Node.js、Deno、Cloudflare Workers 等提供跨平台支持。
