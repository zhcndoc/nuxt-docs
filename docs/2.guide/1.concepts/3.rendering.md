...
[中间件](/docs/guide/directory-structure/app/middleware) 和 [页面](/docs/guide/directory-structure/app/pages) 在服务器和水合过程中在客户端中执行。[插件](/docs/guide/directory-structure/plugins) 可以在服务器、客户端或两者上渲染。[组件](/docs/guide/directory-structure/app/components) 也可以被强制仅在客户端运行。[组合函数](/docs/guide/directory-structure/app/composables) 和 [工具](/docs/guide/directory-structure/app/utils) 的渲染根据它们的使用上下文。
