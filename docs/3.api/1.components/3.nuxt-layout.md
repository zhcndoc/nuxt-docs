...
- `name`: 指定要渲染的布局名称，可以是字符串、响应式引用或计算属性。它**必须**与 [`app/layouts/`](/docs/4.x/guide/directory-structure/app/layouts) 目录中对应布局文件的名称相匹配。
  - **类型**: `string`
  - **默认**: `default`
...
- `fallback`: 如果向 `name` 属性传递了无效的布局，则不会渲染任何布局。在这种情况下，可指定一个 `fallback` 布局进行渲染。它**必须**与 [`app/layouts/`](/docs/4.x/guide/directory-structure/app/layouts) 目录中对应布局文件的名称相匹配。
  - **类型**: `string`
  - **默认**: `null`
