...
`<NuxtPage>` 是 Nuxt 提供的内置组件，用于显示位于 [`app/pages/`](/docs/4.x/guide/directory-structure/app/pages) 目录下的顶层或嵌套页面。

::note
`<NuxtPage>` 是对 Vue Router 中的 [`<RouterView>`](https://router.vuejs.org/api/interfaces/RouterViewProps.html#interface-routerviewprops) 的封装。应当使用 `<NuxtPage>` 而不是 `<RouterView>`，因为前者会额外处理内部状态。否则，`useRoute()` 可能会返回不正确的路径。
::
