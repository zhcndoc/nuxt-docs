...
Nuxt 由 [Nitro](https://nitro.zhcndoc.com) 服务器引擎驱动。使用 Nitro，您可以将高级逻辑直接集成到您的配置中，这对于重定向、代理、缓存和向路由添加头等操作非常有用。这种配置通过将路由模式与特定的路由设置关联起来。

::tip
您可以在 [Nitro 文档](https://nitro.zhcndoc.com/guide/routing#route-rules) 中阅读更多关于 Nitro 路由规则的信息。
::
