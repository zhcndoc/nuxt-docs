...
::note
为方便起见，在预览模式下，你的 [`.env`](/docs/4.x/guide/directory-structure/env) 文件将被加载到 `process.env` 中。（但是在生产环境中你需要自己确保环境变量已被设置。例如，在 Node.js 20+ 中，你可以通过运行 `node --env-file .env .output/server/index.mjs` 来启动服务器。）
::