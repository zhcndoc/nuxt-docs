...
`prepare` 命令会在你的应用中创建一个 [`.nuxt`](/docs/4.x/guide/directory-structure/nuxt) 目录并生成类型。这在 CI 环境中或作为你 [`package.json`](/docs/4.x/guide/directory-structure/package) 中的 `postinstall` 命令时很有用。
