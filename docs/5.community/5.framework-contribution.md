...
阅读完 [general contribution guide](/docs/4.x/community/contribution) 后，这里有一些关于向 [`nuxt/nuxt`](https://github.com/nuxt/nuxt) 仓库贡献的具体说明。
...
1. 将 [`nuxt/nuxt`](https://github.com/nuxt/nuxt) 仓库 [Fork](https://help.github.com/articles/fork-a-repo) 到您自己的 GitHub 账号，然后将其 [克隆](https://help.github.com/articles/cloning-a-repository) 到本地设备。
2. 确保使用最新的 [Node.js](https://nodejs.org/en)（20.x）
3. 启用 [Corepack](https://github.com/nodejs/corepack) 以使用 `pnpm` 和 `yarn`
    ```bash [Terminal]
    corepack enable
    ```
4. 运行 `pnpm install --frozen-lockfile` 使用 pnpm 安装依赖：
    ```bash [Terminal]
    pnpm install --frozen-lockfile
    ```
    ::note
    如果您要添加依赖，请使用 `pnpm add`。 :br
    `pnpm-lock.yaml` 文件是所有 Nuxt 依赖的事实来源。
    ::
5. 激活被动开发系统
    ```bash [Terminal]
    pnpm dev:prepare
...
::important
请确保不要将其提交到您的分支，但在 PR 描述中添加一些示例代码可能会很有帮助。这样可以帮助审阅者和其他 Nuxt 用户更深入地理解您所实现的功能。
::
...
::important
请确保根据 [Documentation Style Guide](/docs/4.x/community/contribution#documentation-style-guide) 进行更改。
::
