...
对 Nuxt 本身进行较大改动时，建议先[创建一个 Nuxt 模块](#create-a-module)实现功能原型，再[以讨论形式创建 RFC](#make-an-rfc)。用户采用并反馈后，功能可以完善并合入 Nuxt 核心或继续作为独立模块。
...
发起 PR 时，请确保标题也遵守[提交规范](#commit-conventions)。
...
我们强烈建议先[创建模块](#create-a-module)验证大型新功能并获得社区采用。
...
由于 ESLint 已配置代码格式化，无需再使用 Prettier 重复功能。你可以运行 `yarn lint --fix`，`pnpm lint --fix`，或 `bun run lint --fix` 来格式化代码，或参考[ESLint 章节](#use-eslint)进行 IDE 设置。
