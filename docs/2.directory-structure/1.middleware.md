...
* `return navigateTo('/')` - 重定向到指定路径，若在服务器端执行重定向，则状态码为 [`302` Found](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/302)
* `return navigateTo('/', { redirectCode: 301 })` - 重定向到指定路径，若在服务器端执行重定向，则状态码为 [`301` Moved Permanently](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/301)
