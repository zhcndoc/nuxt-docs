...
::tip
只有在你没有禁用 `nitro.prerender.crawlLinks`，并且在 `nitro.prerender.routes` 列表中至少有一个页面时，Pages 才会自动注册用于预渲染。请参阅[预渲染指南](/docs/3.x/getting-started/prerendering)。
::
...
路由组会自动在路由元信息中以 `route.meta.groups` 的形式提供。这样你可以在组件中访问组信息，用于条件逻辑、样式或其他用途。
