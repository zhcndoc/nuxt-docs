...
::note
只有目录顶层的文件（或任意子目录中的 index 文件）会被注册。index 文件会使用其所在文件夹的名称：`middleware/auth/index.ts` 会被注册为 `auth`。
::

::note
路由中间件在 Nuxt 应用的 Vue 部分中运行。尽管名称相似，但它们与 [服务器中间件](/docs/4.x/directory-structure/server#server-middleware) 完全不同，后者在应用的 Nitro 服务器部分运行。
::
