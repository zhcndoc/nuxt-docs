...
::note
只有目录顶层的文件（或任何子目录中的 index 文件）会被注册。index 文件会使用其所在文件夹的名称：`middleware/auth/index.ts` 会被注册为 `auth`。
::

::note
路由中间件在 Nuxt 应用的 Vue 部分中运行。尽管名称相似，但它们与 [server middleware](/docs/4.x/directory-structure/server#server-middleware) 完全不同，后者在应用的 Nitro server 部分中运行。
::
...
与 `vue-router` 的 [导航守卫](https://router.vuejs.org/guide/advanced/navigation-guards#Global-Before-Guards) 不同，中间件不会传入第三个 `next()` 参数，重定向或取消路由是通过从中间件返回一个值来处理的。

可能的返回值有：

* 无返回（简单的 `return` 或根本不返回） - 不会阻止导航，会继续下一个中间件（如果有）或完成路由导航
* `return navigateTo('/')` - 重定向到给定路径；若重定向在服务器端发生，则会将重定向状态码设置为 [`302` Found](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/302)
* `return navigateTo('/', { redirectCode: 301 })` - 重定向到给定路径；若重定向在服务器端发生，则会将重定向状态码设置为 [`301` Moved Permanently](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Status/301)
...
::important
我们建议使用上面的辅助函数来执行重定向或停止导航。其他在 [vue-router 文档](https://router.vuejs.org/guide/advanced/navigation-guards#Global-Before-Guards) 中描述的可能返回值可能也能工作，但在将来可能会有破坏性变更。
::
