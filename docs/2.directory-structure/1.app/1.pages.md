...
::tip
只有在您没有禁用 `nitro.prerender.crawlLinks`，并且在 `nitro.prerender.routes` 列表中至少有一个页面时，页面才会自动注册用于预渲染。请参阅[预渲染指南](/docs/4.x/getting-started/prerendering)。
::
...
单个路由可以通过为每个出口指定一个 `name` 并为每个名称提供一个同级页面文件，在父组件中的多个 `<NuxtPage>` 出口中渲染。

使用 `name@view.vue` 文件名约定，在默认路由文件旁边声明一个命名视图：
...
当用户导航到 `/parent/child` 时，`child.vue` 会渲染到默认的 `<NuxtPage />` 中，而 `child@sidebar.vue` 会渲染到 `<NuxtPage name="sidebar" />` 中。没有匹配命名视图的出口将保持为空。

::note
`definePageMeta` 仅从默认路由文件中读取。`name@view.vue` 同级文件中声明的元数据不会对该路由生效。
::
...
### 访问路由分组 :badge[v4.3]{color="info" size="xs" class="align-middle"}
