...
::tip
如果您未禁用 `nitro.prerender.crawlLinks`，并且在 `nitro.prerender.routes` 列表中至少有一个页面，页面才会自动注册用于预渲染。请参阅[预渲染指南](/docs/4.x/getting-started/prerendering)。
::
...
## 命名视图 :badge[v4.5]{color="info" size="xs" class="align-middle"}

单个路由可以通过为每个出口赋予一个 `name`，并为每个名称提供一个同级页面文件，从而在父组件中的多个 `<NuxtPage>` 出口中渲染。

使用 `name@view.vue` 文件命名约定，在默认路由文件旁声明一个命名视图：
...
当用户导航到 `/parent/child` 时，`child.vue` 会渲染到默认的 `<NuxtPage />` 中，而 `child@sidebar.vue` 会渲染到 `<NuxtPage name="sidebar" />` 中。没有匹配命名视图的出口会保持为空。

::note
`definePageMeta` 仅从默认路由文件中读取。`name@view.vue` 同级文件中声明的元数据不会影响该路由。
::

:read-more{title="命名视图" to="https://router.vuejs.org/guide/essentials/named-views.html" target="_blank"}
...
### 访问路由组 :badge[v4.3]{color="info" size="xs" class="align-middle"}
...
随着应用变得越来越大、越来越复杂，您的路由可能需要更多灵活性。因此，Nuxt 直接暴露了路由器、路由和路由器选项，以便以不同方式进行自定义。

:read-more{to="/docs/4.x/guide/recipes/custom-routing"}

## Multiple pages directories

By default, all pages should be located in the project's root `app/pages` directory.

However, you can use [Nuxt layers](/docs/4.x/getting-started/layers) to create groups of application pages:
