...
你也可以在 route rules 中使用 `appLayout` 属性为特定路由设置布局 :badge[v4.3]{color="info" size="xs" class="align-middle"}:
...
Pass props to the `layout` property using object syntax, directly from your page:
...
::tip
Props are fully typed based on the layout's `defineProps`. You'll get autocomplete and type checking in your editor.
::

### Via `setPageLayout`

You can also pass props when changing layouts dynamically, using [`setPageLayout`](/docs/4.x/api/utils/set-page-layout):
