...
description: 使用 composables/ 目录将你的 Vue 可组合函数自动导入到应用中。
...
At the bottom level, Nuxt automatically generates the file `.nuxt/imports.d.ts` to declare types.

Please note that you need to run [`nuxt prepare`](/docs/4.x/api/commands/prepare), [`nuxt dev`](/docs/4.x/api/commands/dev), or [`nuxt build`](/docs/4.x/api/commands/build) in order for Nuxt to generate these types.

::note
If you create a composable without starting the development server, TypeScript will throw an error, for example `Cannot find name 'useBar'.`
::
