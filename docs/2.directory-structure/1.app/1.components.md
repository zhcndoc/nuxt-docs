...
To dynamically import a component (also known as a lazy-loaded component), simply add the `Lazy` prefix to the component name. This is especially useful when a component is not always needed.

By using the `Lazy` prefix, you can defer loading the component code until the appropriate time, which helps optimize the size of your JavaScript bundle.
...
每个目录条目也接受 `pattern` 和 `ignore` glob 选项，它们用于控制在 `path` 中扫描哪些文件。当你的组件位于不遵循默认布局的嵌套文件夹中时，这很有用，例如一种领域驱动的结构：
...
::note
如果指定了 `pattern`，`extensions` 选项将不起作用，因此请确保你的 pattern 能匹配你想要扫描的文件扩展名。
::
...
::note
服务端组件的 props 来自请求（URL 查询或 body），因此应将其视为不可信输入。Nuxt 会拒绝最有可能无意中泄漏的 props：island 未声明的顶层 `as`（未声明的 prop 会作为属性透传到 island 的根元素上），以及在启用 `vue.runtimeCompiler` 时，props 中任意位置出现的 `template`。除此之外，避免将未经过验证的 props 传入动态组件解析（`<component :is>`{lang=vue}、`h()`、`resolveDynamicComponent()`，或多态的 `as` / `asChild` prop），因为字符串可以解析为任何已注册组件或 HTML 元素。

组件未声明的 props 会作为属性透传到其单一根元素上，因此根元素是多态组件（例如来自 `reka-ui` / `@nuxt/ui`）的 island 可能会接收到你未绑定的属性。请在这类 island 上设置 `defineOptions({ inheritAttrs: false })`{lang=ts}，或者声明你接受的 props。

如果要根据调用方输入切换组件，请通过导入组件的允许列表映射一个判别值，而不是直接传递原始 prop：
...
::warning
仅在本地的 `.vue` 单文件组件上使用 `nuxt-client`。像 [`<NuxtLink>`](/docs/4.x/api/components/nuxt-link) 这样的内置组件会跳过 islands 转换。在客户端导航后，你可能会看到 `Failed to locate Teleport target`，或者链接无报错地消失。请将内置组件包裹在你自己的 `.vue` 文件中，并在该包装组件上添加 `nuxt-client`，或者从内置组件上移除 `nuxt-client`。参见 [#29251](https://github.com/nuxt/nuxt/issues/29251) 和 [#26002](https://github.com/nuxt/nuxt/issues/26002)。
::
...
* 你无法从应用的其余部分访问该 “island 上下文”，也无法从 island 组件访问应用的其余上下文。换句话说，服务端组件或 island 与应用的其余部分是隔离的。
...
::important
`useRoute()` 和其他 `vue-router` 组合式函数不会跟踪服务端（island）组件内部的当前页面路由。该 island 会在其自己独立的 Vue 应用中渲染，并且只以其 props（以及任何显式上下文）为键，这也是 islands 能够独立于其所渲染页面进行缓存的原因。在 island 内部，`useRoute()` 反映的是 island 自己的请求，而不是用户当前所在的页面。

如果 island 需要当前路由信息，请显式传入——要么作为父组件的 props，要么通过 `<NuxtIsland>` 上的 `context` prop 传入（在 island 内部可通过 `nuxtApp.ssrContext.islandContext` 读取）。
::

在 island 组件内部，你可以通过 `nuxtApp.ssrContext.islandContext` 访问其 island 上下文。请注意，尽管 island 组件仍处于实验阶段，但该上下文的格式可能会发生变化。
