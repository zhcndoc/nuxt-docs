...
如果你想在目录中对组件进行分组，但不影响其名称，可以使用括号 `(` `)` 将分组目录命名。
...
如果你想仅根据组件名称而不是路径来自动导入组件，那么你需要使用配置对象的扩展形式，将 `pathPrefix` 选项设置为 `false`：
...
懒加载组件有助于控制应用中的 chunk 大小，但它们并不总是能提升运行时性能，因为除非条件渲染，否则它们仍会被主动加载。在真实应用中，一些页面可能包含大量内容和组件，而在大多数情况下，并非所有组件在页面加载时都需要交互。让它们全部主动加载可能会对性能产生负面影响。
...
每个目录条目也接受 `pattern` 和 `ignore` glob 选项，它们用于控制在 `path` 中扫描哪些文件。当你的组件位于不符合默认布局的嵌套文件夹中时，这很有用，例如领域驱动结构：
...
::note
如果指定了 `pattern`，`extensions` 选项将不起作用，因此请确保你的模式匹配你想要扫描的文件扩展名。
::
...
将仅服务端组件使用 `.server` 后缀注册，并可在应用中的任何地方自动使用它们。
...
阅读有关 islands 如何渲染、隔离的 island 上下文、使用 `nuxt-client` 的选择性 hydration、插槽、缓存以及当前限制的更多内容，请参阅专门的服务端组件指南。
::
