...
完全推断出的类型仅在应用代码中可用（组件、composables、插件等）。在服务器路由、`shared/` 目录中的代码以及 `nuxt.config` 中，`app.config` 文件里定义的键会被类型化为 `unknown`；因为要在那里进行类型检查，就需要在应用上下文之外检查您的应用代码，而此时应用自动导入不可用。`nuxt.config` 的 `appConfig` 选项中内联定义的键会在所有地方都被类型化。如果您需要在应用代码之外为其他键添加类型，您可以按下面所述扩展 `AppConfig` 接口。增强文件所在的位置决定了哪些上下文可以看到它：`shared/` 目录中的 `.d.ts` 文件会覆盖应用代码、共享代码和服务器路由。

不过，在某些情况下，您可能希望自己为其添加类型。您可能想要类型化的内容有两种。
...
declare module 'nuxt/schema' {
  interface AppConfig {
    ...
    theme: {
      // 您可能希望为该值添加类型，以提供比 Nuxt 能推断出的更具体的类型，
      // 例如字符串字面量类型
