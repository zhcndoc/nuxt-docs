...
::important
不要在您的服务器路由或工具中导入 Vue 应用代码（组件、composables 或其他仅限应用的工具），也不要在您的应用中导入仅限服务器的代码。
::

:read-more{to="/docs/4.x/directory-structure/shared#why-you-cannot-mix-vue-and-nitro-code" title="为什么不能混用 Vue 和 Nitro 代码"}
...
中间件处理程序将在每个请求上在任何其他服务器路由之前运行，用于添加或检查标头、记录请求或扩展事件的请求对象。
...
放置在 `~~/server/types/` 中的类型仅在服务器上下文中自动导入，因此你可以在服务器路由、中间件、插件和工具函数中引用它们，而无需导入。Vue 应用中也需要使用的类型应放在 [`shared/types/`](/docs/4.x/directory-structure/shared) 中。
...
只有直接位于 `server/types/` 中的文件会被扫描；嵌套子目录中的文件不会被自动导入，这与 [`shared/types/`](/docs/4.x/directory-structure/shared#how-files-are-scanned) 的工作方式一致。
...
或者，使用 `getValidatedRouterParams` 配合 Zod 或 Valibot 等 schema 验证器，以实现运行时和类型安全。
::
...
例如，创建名为 `~/server/api/foo/[...].ts` 的文件将为所有未匹配到任何路由处理程序的请求注册一个全捕获路由，例如 `/api/foo/bar/baz`。
...
您可以通过使用 `~/server/api/foo/[...slug].ts` 为全捕获路由设置名称，并通过 `event.context.params.slug` 访问它。
...
或者，使用 `readValidatedBody` 配合 Zod 或 Valibot 等 schema 验证器，以实现运行时和类型安全。
::
...
::tip{to="https://unjs.io/blog/2023-08-15-h3-towards-the-edge-of-the-web#runtime-type-safe-request-utils"}
或者，使用 `getValidatedQuery` 配合 Zod 或 Valibot 等 schema 验证器，以实现运行时和类型安全。
::
