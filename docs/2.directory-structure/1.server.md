...
处理程序可以直接返回 JSON 数据、一个 `Promise`，或一个 `Response` 对象。

::important
不要在您的服务器路由或工具中导入 Vue 应用代码（组件、可组合函数或其他仅限应用的工具），也不要在您的应用中导入仅限服务器端的代码。
::

:read-more{to="/docs/4.x/directory-structure/shared#why-you-cannot-mix-vue-and-nitro-code" title="为什么不能混合 Vue 和 Nitro 代码"}
...
export default defineEventHandler(() => '你好，世界！')
```

根据上面的示例，`/hello` 路由将在 <http://localhost:3000/hello> 可访问。

::note
请注意，目前服务器路由尚不支持像 [pages](/docs/guide/directory-structure/pages#dynamic-routes) 那样的动态路由的全部功能。
::
...
::note
中间件处理程序不应返回任何内容（也不应关闭或响应请求），仅应检查或扩展事件上下文或抛出错误。
::
...
export default defineWrappedResponseHandler(event => '你好，世界')
```

## 服务器别名 :badge[v4.3]{color="info" size="xs" class="align-middle"}

您可以使用 `#server` 别名从 `server/` 目录的任意位置导入 `server/` 目录中的文件。
...
默认情况下，Nuxt 4 会生成一个包含 [`tsconfig.json`](/docs/guide/directory-structure/tsconfig) 的项目引用，其中涵盖 `server/` 文件夹以确保类型准确性。

放置在 `~~/server/types/` 中的类型仅会在服务端上下文中自动导入，因此你可以在服务端路由、中间件、插件和工具中直接引用它们，而无需导入。Vue 应用中也需要用到的类型应放在 [`shared/types/`](/docs/4.x/directory-structure/shared) 中。
...
只会扫描直接位于 `server/types/` 中的文件；嵌套子目录中的文件不会被自动导入，这与 [`shared/types/`](/docs/4.x/directory-structure/shared#how-files-are-scanned) 的工作方式一致。
...
或者，使用 `getValidatedRouterParams` 搭配 Zod 或 Valibot 等模式验证器，以确保运行时和类型安全。
::

现在您可以在 `/api/hello/nuxt` 调用此 API 并获得 `你好，nuxt！`。
...
您也可以在目录中使用 `index.[method].ts` 以不同方式组织代码，这在创建 API 命名空间时很有用。
...
例如，创建名为 `~/server/api/foo/[...].ts` 的文件将为所有未匹配到任何路由处理程序的请求注册一个通配路由，例如 `/api/foo/bar/baz`。
...
或者，使用 `readValidatedBody` 搭配 Zod 或 Valibot 等模式验证器，以确保运行时和类型安全。
::
...
::tip{to="https://unjs.io/blog/2023-08-15-h3-towards-the-edge-of-the-web#runtime-type-safe-request-utils"}
或者，使用 `getValidatedQuery` 搭配 Zod 或 Valibot 等模式验证器，以确保运行时和类型安全。
::
...
要返回其他错误代码，请使用 [`createError`](/docs/guide/concepts/server-engine#error-handling) 抛出异常：
...
要返回其他状态码，请使用 [`setResponseStatus`](/docs/guide/concepts/server-engine#setresponsestatus) 实用函数。
...
router.get('/test', defineEventHandler(() => '你好世界'))
...
Nitro 提供了一个跨平台的 [存储层](https://nitro.unjs.io/guide/storage)。为了配置额外的存储挂载点，您可以使用 `nitro.storage` 或 [服务器插件](/docs/guide/directory-structure/server#server-plugins)。
