...
要添加子目录中的插件，可以在 `nuxt.config.ts` 中使用 [`plugins`](/docs/3.x/api/nuxt-config#plugins-1) 选项：
...
插件唯一传入的参数是 [`nuxtApp`](/docs/3.x/api/composables/use-nuxt-app)。
...
你可以在 Nuxt 插件中使用 [组合式函数](/docs/3.x/directory-structure/composables) 和 [工具函数](/docs/3.x/directory-structure/utils)：
...
通常，Vue.js 组合式函数绑定当前组件实例，而插件仅绑定 [`nuxtApp`](/docs/3.x/api/composables/use-nuxt-app) 实例。
::

## 提供帮助函数

如果你想在 [`NuxtApp`](/docs/3.x/api/composables/use-nuxt-app) 实例上提供帮助函数，可以从插件中通过 `provide` 属性返回。
...
::important
强烈建议使用 [`composables`](/docs/3.x/directory-structure/composables) 替代直接提供帮助函数，以避免污染全局命名空间并保持主包体积尽可能小。
::
...
::note
如果你需要在另一个插件中使用已提供的帮助函数，可以调用 [`useNuxtApp()`](/docs/3.x/api/composables/use-nuxt-app) 获取具备类型的版本。但一般情况下除非确保插件加载顺序，否则应避免这样做。
::
