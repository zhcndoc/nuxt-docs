...
`import type` 会在编译时被擦除，不会把运行时代码拉入另一个包中，因此跨边界只导入类型看起来可能可行。即便如此，也请把共享类型（例如 API 响应类型）放在 `shared/types/` 中，因为它们会在两个上下文中自动导入。这样可以保持边界清晰，避免以后不小心把类型导入变成值导入，并且也符合 Nuxt 为应用、服务器和共享代码分别设置的 [类型上下文](/docs/4.x/guide/concepts/typescript#project-references)。

仅在一个上下文中使用的类型可以放在对应的上下文旁边：`app/types/` 仅在 Vue 应用中自动导入，而 [`server/types/`](/docs/4.x/directory-structure/server#server-types) 仅在 Nitro 服务器中自动导入。当一种类型在两个上下文中都需要时，请使用 `shared/types/`。
