...
::important
此文件应添加到你的 [`.gitignore`](/docs/4.x/directory-structure/gitignore) 文件中，以避免将密钥推送到你的仓库。
::

## 开发、构建和生成时

Nuxt CLI 在开发模式下，以及运行 [`nuxt build`](/docs/4.x/api/commands/build) 和 [`nuxt generate`](/docs/4.x/api/commands/generate) 时，通过 [c12](https://github.com/unjs/c12) 内置支持 `.env`。
...
::important
在应用代码中，你应使用 [运行时配置](/docs/4.x/guide/going-further/runtime-config) 而不是直接使用普通的环境变量。
::
