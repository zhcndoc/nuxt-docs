...
::note
为了更清晰起见，建议组件的文件名与其名称匹配。所以，在上述示例中，你可以将 `Button.vue` 重命名为 `BaseFooButton.vue`。
::
...
::important
如果您使用 `resolveComponent` 来处理动态组件，请确保只插入组件的名称，该名称必须是字面量字符串，不能是变量或包含变量。该字符串会在编译阶段进行静态分析。
::
...
::note
服务端组件的 props 来自请求（URL 查询或 body），因此应将其视为不可信输入。Nuxt 会拒绝最可能无意间泄漏的 props：岛屿未声明的顶层 `as`（未声明的 prop 会作为属性透传到岛屿的根节点），以及在启用 `vue.runtimeCompiler` 时，props 中任意位置的 `template`。除此之外，避免将未验证的 props 传入动态组件解析（`<component :is>`{lang=vue}、`h()`、`resolveDynamicComponent()`，或多态的 `as` / `asChild` prop），因为字符串可以解析为任何已注册组件或 HTML 元素。

组件未声明的 props 会作为属性透传到其单个根元素，因此根元素是多态组件（例如来自 `reka-ui` / `@nuxt/ui`）的岛屿可能会接收到你未绑定的属性。请在这类岛屿上设置 `defineOptions({ inheritAttrs: false })`{lang=ts}，或声明你接受的 props。

若要根据调用方输入切换组件，请通过允许列表将判别值映射到已导入的组件，而不要直接传递原始 prop：
...
::important
`useRoute()` 和其他 `vue-router` 组合式函数不会在服务端（岛屿）组件内跟踪当前页面路由。该岛屿是在其自身隔离的 Vue 应用中渲染的，并且只以它的 props（以及任何显式上下文）作为键，这正是让岛屿可以独立于所渲染页面进行缓存的原因。在岛屿内，`useRoute()` 反映的是岛屿自己的请求，而不是用户当前所在的页面。

如果某个岛屿需要当前路由的信息，请显式传入——可以作为父组件的 props 传递，或通过 `<NuxtIsland>` 上的 `context` 属性传递（在岛屿内部可从 `nuxtApp.ssrContext.islandContext` 读取）。
::

在岛屿组件内部，你可以通过 `nuxtApp.ssrContext.islandContext` 访问其岛屿上下文。请注意，虽然岛屿组件仍处于实验阶段，但该上下文的格式未来可能会变化。
