...
你可以通过将 `experimental.emitRouteChunkError` 设置为 `false`（完全禁用对这些错误的钩子处理）或设置为 `manual`（如果你希望自行处理）来更改此行为。如果你想手动处理 chunk 加载错误，可以参考[自动实现](https://github.com/nuxt/nuxt/blob/main/packages/nuxt/src/app/plugins/chunk-reload.client.ts)来获取思路。
...
<!-- TODO：Twoslash 尚不支持 tsconfig 路径 -->
...
在开发环境中，错误的 `cause` 会被保留并暴露给你的错误页面，以便你追踪原始错误；在生产环境中，错误响应或错误页面负载中永远不会包含 `cause`。
