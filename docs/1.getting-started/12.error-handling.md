...
此外，Nuxt 还提供了 [`vue:error`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) 钩子，如果任何错误冒泡到顶层，该钩子就会被调用。

如果你使用错误报告框架，可以通过 [`vueApp.config.errorHandler`](https://vue.zhcndoc.com/api/application.html#app-config-errorhandler) 提供全局处理器。它将接收所有 Vue 错误，即使这些错误已经被处理。
...
::note
请注意，`vue:error` 钩子基于 [`onErrorCaptured`](https://vue.zhcndoc.com/api/composition-api-lifecycle.html#onerrorcaptured) 生命周期钩子。
::
...
你可以通过将 `experimental.emitRouteChunkError` 设置为 `false`（完全禁用对这些错误的钩取）或设置为 `manual`（如果你想自行处理）来更改此行为。如果你想手动处理 chunk 加载错误，可以参考[自动实现](https://github.com/nuxt/nuxt/blob/main/packages/nuxt/src/app/plugins/chunk-reload.client.ts)来获取思路。
...
<!-- TODO:twoslash：Twoslash 尚不支持 tsconfig 路径 -->
