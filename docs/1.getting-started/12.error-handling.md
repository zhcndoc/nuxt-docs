...
You can use the [`onErrorCaptured`](https://vue.zhcndoc.com/api/composition-api-lifecycle.html#onerrorcaptured) hook to capture Vue errors.

In addition, Nuxt provides a [`vue:error`](/docs/4.x/api/advanced/hooks#app-hooks-runtime) hook, which will be called if any error bubbles up to the top level.

If you are using an error reporting framework, you can provide a global handler via [`vueApp.config.errorHandler`](https://vue.zhcndoc.com/api/application.html#app-config-errorhandler). It will receive all Vue errors, even if those errors have already been handled.

```ts twoslash [plugins/error-handler.ts]
export default defineNuxtPlugin((nuxtApp) => {
  nuxtApp.vueApp.config.errorHandler = (error, instance, info) => {
    // Handle errors, for example report them to a service
  }
...
::note
Please note that the `vue:error` hook is based on the [`onErrorCaptured`](https://vue.zhcndoc.com/api/composition-api-lifecycle.html#onerrorcaptured) lifecycle hook.
::
...
你可能会遇到 chunk 加载错误，原因是网络连接失败或新的部署（使旧的带哈希的 JS chunk URL 失效）。Nuxt 内置了对 chunk 加载错误的处理支持：在路由导航期间如果某个 chunk 加载失败，会执行硬刷新。
...
在开发环境中，错误的 `cause` 会被保留并暴露到你的错误页面，以便你追踪原始错误；在生产环境中，响应或错误页面载荷中绝不会包含 `cause`。
...
::tip
`statusText` 属性适用于简短的、符合 HTTP 标准的状态文本（例如，“未找到”）。它应仅包含水平制表符、空格和可见的 ASCII 字符（`[\t\u0020-\u007E]`）。

对于任何详细描述、多行消息或包含非 ASCII 字符的内容，应始终使用 `message` 属性。
::
