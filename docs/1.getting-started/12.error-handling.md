...
你可以使用 [`onErrorCaptured`](https://vuejs.org/api/composition-api-lifecycle#onerrorcaptured) 钩子捕获 Vue 错误。

此外，Nuxt 提供了一个 [`vue:error`](/docs/api/advanced/hooks#app-hooks-runtime) 钩子，如果有错误冒泡到顶层，该钩子会被调用。

如果你使用错误报告框架，可以通过 [`vueApp.config.errorHandler`](https://vuejs.org/api/application#app-config-errorhandler) 提供全局处理函数。它会接收所有 Vue 错误，即使这些错误已被处理。
...
::note
注意，`vue:error` 钩子是基于 [`onErrorCaptured`](https://vuejs.org/api/composition-api-lifecycle#onerrorcaptured) 生命周期钩子的。
::
...
目前你无法为这些错误定义服务器端处理程序，但可以渲染错误页面，详见 [渲染错误页面](/docs/3.x/getting-started/error-handling#error-page) 部分。
...
::note
渲染错误页面是一个全新的页面加载，这意味着所有已注册的中间件会再次执行。你可以在中间件中使用 [`useError`](/docs/3.x/getting-started/error-handling#useerror) 来检测是否正在处理错误。
::
...
- 服务器端会触发全屏错误页面，你可以用 [`clearError`](/docs/3.x/getting-started/error-handling#clearerror) 清除。
...
::tip
`statusText` 属性用于简短的、符合 HTTP 标准的状态文本（例如 "Not Found"）。它应只包含水平制表符、空格和可见的 ASCII 字符 (`[\t\u0020-\u007E]`)。

对于任何详细描述、多行消息或含非 ASCII 字符的内容，应始终使用 `message` 属性。
::
...
你可以在客户端的任何时刻调用此函数，或在服务器端的中间件、插件或 `setup()` 函数中直接调用。它会触发全屏错误页面，你可以通过 [`clearError`](/docs/3.x/getting-started/error-handling#clearerror) 清除。
