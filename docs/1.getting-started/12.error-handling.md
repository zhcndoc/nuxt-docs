...
您可以使用 [`onErrorCaptured`](https://vuejs.org/api/composition-api-lifecycle.html#onerrorcaptured) 来捕获 Vue 错误。

此外，Nuxt 还提供了一个 [`vue:error`](/docs/api/advanced/hooks#app-hooks-runtime) 钩子，如果任何错误传播到顶层，它将被调用。

如果您使用错误报告框架，可以通过 [`vueApp.config.errorHandler`](https://vuejs.org/api/application.html#app-config-errorhandler) 提供全局处理器。它将接收所有 Vue 错误，即使它们已被处理。
