...
3. 如果你的 Nuxt 环境测试位于 `test/nuxt/` 之外，请参阅 [测试中的 TypeScript 支持](#typescript-support-in-tests) 以将它们添加到 TypeScript 上下文中。
...
> **注意**：如果你的组件中的请求指向外部 API，你可以使用 `baseURL`，然后使用 [Nuxt 环境覆盖配置](/docs/4.x/getting-started/configuration#environment-overrides)（`$test`）将其置空，这样你的所有请求都会 go 到 Nitro 服务器。
