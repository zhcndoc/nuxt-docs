...
我们当前为需要 Nuxt 运行时环境的代码提供了单元测试环境。目前「仅支持 `vitest`」（欢迎贡献以添加其他运行时）。
...
2. 创建一个包含以下内容的 `vitest.config.ts`：
   ...
   ```
    ...
    ::note
    `defineVitestProject` 仅用于 Nuxt 环境测试。端到端测试应配置为常规的 `test.environment: 'node'` 项目。
    ::

3. 如果你的 Nuxt 环境测试位于 `test/nuxt/` 目录之外，请参阅[测试中的 TypeScript 支持](#typescript-support-in-tests)，将它们添加到 TypeScript 上下文中。

::tip
在你的 vitest 配置中导入 `@nuxt/test-utils` 时，必须在 `package.json` 中指定 `"type": "module"`，或适当地重命名你的 vitest 配置文件。
...
默认为 `true`，创建一个不具备功能的 IntersectionObserver API 占位类。

#### `indexedDB`

默认为 `false`，使用 [`fake-indexeddb`](https://github.com/dumbmatter/fakeIndexedDB) 创建一个功能完整的 IndexedDB API 模拟。
...
该选项对象接受 `@vue/test-utils` 的挂载选项以及以下属性：

  - `route`：初始路由，或设为 `false` 以跳过初始路由切换（默认 `/`）。
...
该选项对象接受 `@testing-library/vue` 的渲染选项以及以下属性：
...
你可以显式地为 mock 指定类型以获得类型安全，并在模拟复杂功能时使用传入工厂函数的原始实现。
...
如果你需要在不同测试之间为 Nuxt 导入提供不同的实现，可以通过创建并暴露你的模拟（使用 [`vi.hoisted`](https://vitest.dev/api/vi#vi-hoisted)）来实现，然后在 `mockNuxtImport` 中使用这些模拟。这样你可以访问被模拟的导入，并在测试之间更改实现。注意在每个测试前后恢复模拟状态以撤销模拟的状态更改（参见 [恢复 mock](https://vitest.dev/api/mock#mockrestore)）。
...
- `logLevel`：覆盖服务器子进程的 consola 日志级别。（使用 NUXT_TEST_LOG_LEVEL 环境变量进行覆盖）
  - 类型：`number`
  - 默认：`1`
- `captureServerLogs`：是否捕获服务器进程输出，而不是继承标准输入输出。当设置为 `true`（默认值）时，服务器 stdout/stderr 不会输出到控制台，而是可通过 `getServerLogs()` 访问。设置为 `false` 可恢复旧版的继承标准输入输出行为（适用于在本地调试测试）。
  - 类型：`boolean`
  - 默认：`true`
...
清除捕获的服务器日志行。在请求之间使用此方法，可以仅断言特定操作产生的日志。
...
在 `vitest`、`jest` 或 `cucumber` 中，你可以使用 `createPage` 创建配置好的 Playwright 浏览器实例，并可选择将其指向运行中服务器的某个路径。你可以在 [Playwright 文档](https://playwright.dev/docs/api/class-page)中了解更多可用的 API 方法。
