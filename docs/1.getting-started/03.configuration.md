...
默认情况下，Nuxt 已经配置好以覆盖大多数使用场景。可以使用项目根目录下的 [`nuxt.config.ts`](/docs/4.x/guide/directory-structure/nuxt-config) 文件来覆盖或扩展此默认配置。

## Nuxt 配置

位于 Nuxt 项目根目录的 [`nuxt.config.ts`](/docs/4.x/guide/directory-structure/nuxt-config) 文件可以用来覆盖或扩展应用的行为。
...
Nuxt 使用 [`nuxt.config.ts`](/docs/4.x/guide/directory-structure/nuxt-config) 文件作为配置的单一真实来源，并跳过读取外部配置文件。在构建项目的过程中，你可能需要配置那些工具。下表列出了常见配置项以及在 Nuxt 中（如适用）如何配置它们。
...
Name                                         | Config File             | How To Configure
---------------------------------------------|-------------------------|--------------------------
[TypeScript](https://www.typescriptlang.org) | `tsconfig.json`         | [更多信息](/docs/4.x/guide/directory-structure/tsconfig)
