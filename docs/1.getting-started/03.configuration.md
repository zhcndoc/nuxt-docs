...
默认情况下，Nuxt 配置覆盖了大多数使用场景。可以通过 [`nuxt.config.ts`](/docs/3.x/directory-structure/nuxt-config) 文件重写或扩展默认配置。

## Nuxt 配置

[`nuxt.config.ts`](/docs/3.x/directory-structure/nuxt-config) 文件位于 Nuxt 项目的根目录，可用于重写或扩展应用行为。
...
这些变量通过 [`useRuntimeConfig()`](/docs/3.x/api/composables/use-runtime-config) 组合式 API 暴露给应用其余部分。
...
通过 [`useAppConfig`](/docs/3.x/api/composables/use-app-config) 组合式 API 将这些变量暴露给应用其余部分。
...
Nuxt 使用 [`nuxt.config.ts`](/docs/3.x/directory-structure/nuxt-config) 文件作为唯一配置源，不会读取外部配置文件。在构建过程中，可能需要配置这些文件。以下表格列出常见配置及在 Nuxt 中的配置方式。

名称                                         | 配置文件                 | 配置方式
---------------------------------------------|-------------------------|-------------------------
[Nitro](https://nitro.build)               | ~~`nitro.config.ts`~~    | 在 `nuxt.config` 中使用 [`nitro`](/docs/3.x/api/nuxt-config#nitro) 键
[PostCSS](https://postcss.org)               | ~~`postcss.config.js`~~  | 在 `nuxt.config` 中使用 [`postcss`](/docs/3.x/api/nuxt-config#postcss) 键
[Vite](https://vite.dev)                     | ~~`vite.config.ts`~~     | 在 `nuxt.config` 中使用 [`vite`](/docs/3.x/api/nuxt-config#vite) 键
[webpack](https://webpack.js.org)            | ~~`webpack.config.ts`~~  | 在 `nuxt.config` 中使用 [`webpack`](/docs/3.x/api/nuxt-config#webpack-1) 键
