...
::note
默认启用代码拆分，且推荐大多数应用使用。如果你有明确理由想改为只输出单个 bundle，可以在你的 [`nuxt.config`](/docs/4.x/api/nuxt-config) 中将其禁用：
...
这会将所有 JavaScript 打包到单个文件中。它很少会带来收益——通常会增加初始下载量，即使在慢速网络下也是如此——因此，只有在你已经通过测量确认它对你的场景有帮助时，才应禁用代码拆分。
::
