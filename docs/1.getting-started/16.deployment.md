...
::important
运行服务器时请设置 `NODE_ENV=production`。某些依赖项（尤其是 Vue Router）只有在设置此项时才会去除仅开发环境的警告，因此如果不设置，您的日志可能会被诸如 `[Vue Router warn]: No match found for location with path …` 之类的消息淹没，尤其是在未匹配的路由上。
::
...
:read-more{to="https://nitro.build/deploy/runtimes/node" title="Nitro 文档中 node-server 预设的相关内容"}
...
:read-more{title="Nuxt 预渲染" to="/docs/3.x/getting-started/prerendering"}
...
2. 安全 > 设置 > 禁用 "Email Address Obfuscation"
