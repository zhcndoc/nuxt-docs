...
::important
运行服务器时请设置 `NODE_ENV=production`。某些依赖项（尤其是 Vue Router）只有在设置此项时才会移除仅开发环境下的警告，因此如果不设置，它们可能会用诸如 `[Vue Router warn]: No match found for location with path …` 之类的消息淹没你的日志，尤其是在未匹配的路由上。
::
...
如果反向代理有意将同一个渲染页面暴露在多个公共路径下，Nuxt 可能会在 hydration 期间将浏览器 URL 替换为用于服务端渲染的路径。你可以通过在服务器插件中从 payload 中移除渲染路径来保留浏览器 URL：
...
仅当代理已经处理每个公共路径的资源和路由时，才使用此方法。如果没有渲染路径，Nuxt 将无法修正请求 URL 与服务端渲染路由之间真正的不匹配。
...
- 另外，你也可以使用 `ssr: false`（静态单页应用）来预渲染你的网站。这将生成 HTML 页面，其中包含一个空的 `<div id="__nuxt"></div>`，Vue 应用通常会在这里渲染。这样会失去预渲染网站带来的许多 SEO 优势，因此建议改为使用 [`<ClientOnly>`](/docs/4.x/api/components/client-only) 包裹网站中无法由服务端渲染的部分（如果有的话）。

预渲染的路由还会生成 `_payload.json` 文件，其中包含构建时捕获的数据，Nuxt 会在客户端导航期间复用这些数据。更多内容请阅读 [payload 提取](/docs/4.x/getting-started/prerendering#payload-extraction)。

### 静态回退页面

Nuxt 可以为静态主机生成两个回退页面：

- `200.html` 是单页应用回退页。当你希望由客户端路由处理 URL 时，请将你的主机配置为在未匹配路由时提供此页面。
- `404.html` 是未找到回退页。请将你的主机配置为为应保持 404 状态的路由提供此页面。

`nuxt generate` 和 `nuxt build --prerender` 会自动生成这些文件。如果你使用带有路由规则来预渲染所选路由的 `nuxt build`，请显式添加回退页面：
...
一些提供商使用 `200.html`，一些使用 `404.html`，还有一些允许你配置两者。部署后，请检查你的托管提供商的静态回退或重写设置。
...
除了 Node.js 服务器和静态托管服务外，Nuxt 项目还可以使用多个经过充分测试的预设进行部署，只需极少的配置。
