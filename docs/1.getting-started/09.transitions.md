...
The two keys `pageTransition` and `layoutTransition` both accept [`TransitionProps`](https://vuejs.org/api/built-in-components#transition) values that can be serialized to JSON. You can pass `name`, `mode`, and other valid transition props for custom CSS transitions through them.
