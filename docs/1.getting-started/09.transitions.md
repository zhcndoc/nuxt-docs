...
::important
由于 Nuxt 使用了 Vue 的 `<Transition>` 组件，你想要设置动画的页面或布局必须只有一个 **单一根元素**。包含多个根元素（fragment）的页面或布局无法设置动画，因此过渡将不会运行，并且在路由之间导航时可能会出错。Nuxt 会在开发环境中对此发出警告。请将模板包裹在一个单一根元素中（例如 `<div>`）。
::

## 页面过渡

你可以启用页面过渡，为所有 [页面](/docs/4.x/directory-structure/app/pages) 自动应用过渡。
...
::note
如果你同时更改布局和页面，则此处设置的页面过渡将不会运行。相反，你应该设置一个[布局过渡](/docs/4.x/getting-started/transitions#layout-transitions)。
::
...
该页面现在在跳转到下一个 id 时应用 `slide-left` 过渡，在跳转到上一个 id 时应用 `slide-right` 过渡：
...
当在 `app.vue` 中使用 `<NuxtPage />` 时，可以通过 `transition` 属性配置过渡，以在全局启用过渡。
