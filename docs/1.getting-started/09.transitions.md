...
::note
Nuxt 使用 Vue 的 [`<Transition>`](https://vue.zhcndoc.com/guide/built-ins/transition.html#the-transition-component) 组件在页面和布局之间应用过渡。
::

## 页面过渡

你可以启用页面过渡，为所有 [pages](/docs/4.x/guide/directory-structure/app/pages) 自动应用过渡。
...
要开始为页面之间添加过渡，请将以下 CSS 添加到你的 [`app.vue`](/docs/4.x/guide/directory-structure/app)：
...
你可以启用布局过渡，为所有 [layouts](/docs/4.x/guide/directory-structure/app/layouts) 自动应用过渡。
...
要开始为页面和布局之间添加过渡，请将以下 CSS 添加到你的 [`app.vue`](/docs/4.x/guide/directory-structure/app)：
...
`pageTransition` 和 `layoutTransition` 两个键都接受可 JSON 序列化的 [`TransitionProps`](https://vue.zhcndoc.com/api/built-in-components.html#transition) 值，你可以通过它们传递 `name`、`mode` 以及自定义 CSS 过渡的其他有效 transition-props。
...
::tip
了解有关 `Transition` 组件中可用的额外 [JavaScript 钩子](https://vue.zhcndoc.com/guide/built-ins/transition.html#javascript-hooks) 的更多信息。
::

## 动态过渡

要使用条件逻辑应用动态过渡，你可以利用内联 [middleware](/docs/4.x/guide/directory-structure/app/middleware) 将不同的过渡名称分配给 `to.meta.pageTransition`。
...
Nuxt 提供了对 [**View Transitions API**](https://developer.chrome.com/docs/web-platform/view-transitions)（参见 [MDN](https://developer.mozilla.org/en-US/docs/Web/API/View_Transitions_API)）的实验性实现。这是一种令人兴奋的新方式来实现原生浏览器过渡，它（除其他功能外）能够在不同页面上对不相关的元素进行过渡。
...
默认情况下，视图过渡为所有 [pages](/docs/4.x/guide/directory-structure/app/pages) 启用，但你可以设置不同的全局默认值。
