...
::important
由于 Nuxt 使用的是 Vue 的 `<Transition>` 组件，你想要设置动画的页面或布局必须只有一个**根元素**。具有多个根元素（即 fragment）的页面或布局无法执行动画，因此过渡不会运行，在路由之间导航时可能会报错。Nuxt 会在开发环境中对此发出警告。请将模板包裹在单个根元素中（例如 `<div>`）。
::
...
::note
如果你同时更改布局和页面，那么你在这里设置的页面过渡将不会运行。相反，你应该设置一个[布局过渡](/docs/4.x/getting-started/transitions#layout-transitions)。
::
...
`pageTransition` 和 `layoutTransition` 这两个键都接受可以序列化为 JSON 的 [`TransitionProps`](https://vuejs.org/api/built-in-components#transition) 值。你可以通过它们传入 `name`、`mode` 以及其他有效的过渡属性，以实现自定义 CSS 过渡。
...
::warning
如果更改了 `name` 属性，也必须相应地重命名 CSS 类。
::

要覆盖全局过渡属性，请使用 `definePageMeta` 为单个 Nuxt 页面定义页面或布局过渡，从而覆盖 `nuxt.config` 文件中全局定义的任何页面或布局过渡。
...
该页面现在在前往下一个 id 时应用 `slide-left` 过渡，在前往上一个 id 时应用 `slide-right` 过渡：
