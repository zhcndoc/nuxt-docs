...
* 新的 Nuxt 默认 `srcDir` 默认为 `app/`，大多数内容从此位置解析。
* `serverDir` 现在默认为 `<rootDir>/server` 而不是 `<srcDir>/server`
* `modules` 和 `public` 默认相对于 `<rootDir>` 解析
* 添加了新的 `dir.app`，用于寻找 `router.options.ts` 和 `spa-loading-template.html` - 默认为 `<srcDir>/`
...
👉 更多详情，请参阅 [实施此更改的 PR](https://github.com/nuxt/nuxt/pull/27029)。

##### 变更原因

1. **性能** - 将所有代码放在存储库的根目录中会导致 `.git/` 和 `node_modules/` 文件夹被 FS 监视器扫描/包含，这可能会在非 Mac OS 的情况下显著延迟启动。
2. **IDE 类型安全** - `server/` 和您的应用程序的其他部分在两个完全不同的上下文中运行，并且确保 `server/` 不在与您的应用程序其他部分相同的文件夹内是确保在 IDE 中获得良好的自动完成的重要第一步。

##### 迁移步骤

1. 创建一个名为 `app/` 的新目录。
2. 将 `assets/`、`components/`、`composables/`、`layouts/`、`middleware/`、`pages/`、`plugins/` 和 `utils/` 的文件夹移动到其中，以及 `app.vue`、`error.vue` 和 `app.config.ts`。如果您有 `app/router-options.ts` 或 `app/spa-loading-template.html`，这些路径保持不变。
3. 确保您的 `nuxt.config.ts`、`modules/`、`public/` 和 `server/` 文件夹保持在 `app/` 文件夹之外，即项目的根目录。

然而，**迁移不是必需的**。如果您希望保持当前的文件夹结构，Nuxt 应该可以自动检测到它（如果不能，请提交问题报告）。唯一的例外是如果您已经有一个自定义的 `srcDir`。在这种情况下，您应该知道您的 `modules/`、`public/` 和 `server/` 文件夹将从您的 `rootDir` 解析，而不是从您的自定义 `srcDir` 解析。如果需要，您可以通过配置 `dir.modules`、`dir.public` 和 `serverDir` 来覆盖此行为。
...
迁移指南提供了将 Nuxt 2 功能与 Nuxt 3 功能进行逐步比较的步骤，并指导您如何调整当前应用程序。
...
如果您希望逐渐将您的 Nuxt 2 应用程序迁移到 Nuxt 3，您可以使用 Nuxt Bridge。Nuxt Bridge 是一个兼容层，允许您在 Nuxt 2 中使用 Nuxt 3 的功能，并具有可选的机制。
