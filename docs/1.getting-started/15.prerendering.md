...
Nuxt 会在 HTML 旁边生成 `_payload.json`，用于：
- 预渲染路由（在构建时）
- ISR/SWR 路由（在首次请求时）

Payload 包含来自 `useAsyncData` 和 `useFetch` 的序列化数据。客户端导航会加载这些缓存的 payload，而不是重新获取数据。使用路由规则：为诸如 `pages/[...slug].vue` 这样的动态路由配置 `'/**': { isr: true }`。

::read-more{to="/docs/3.x/api/commands/generate#nuxt-generate"}
了解更多关于 `nuxt generate` 命令的信息。
::
