...
对于更细致的控制，`status` 变量可以是：

- `idle`：当获取还未开始。
- `pending`：当获取已开始，但尚未完成。
- `error`：当获取失败时。
- `success`：当获取成功完成。

## 传递头部和 cookie

当我们在浏览器中调用 `$fetch` 时，用户的头部如 `cookie` 将被直接发送到 API。但在服务器端渲染期间，因 `$fetch` 请求是在服务器内“内部”进行的，因此不会包含用户浏览器中的 cookie，也不会转发 fetch 响应中的 cookies。

### 将客户端头部传递到 API

我们可以使用 [`useRequestHeaders`](/docs/api/composables/use-request-headers) 来访问并将 cookie 从服务器端代理到 API。

以下示例将请求头添加到一个同构的 `$fetch` 调用，以确保 API 端点可以访问用户最初发送的相同 `cookie` 头。
...
::caution
在将头部代理到外部 API 之前，请务必非常小心，仅包括您需要的头部。并非所有头部都可以安全地绕过，可能会引入不必要的行为。以下是常见的并不应被代理的头部列表：
