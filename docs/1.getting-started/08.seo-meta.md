...
::note
当你设置 [`app.cdnURL`](/docs/4.x/api/nuxt-config#cdnurl) 时，你的 [`public/`](/docs/4.x/directory-structure/public) 目录中的资源（包括 `favicon.ico`）会从该 CDN 提供。Nuxt 会根据 `cdnURL` 解析公共资源，并回退到 `app.baseURL`。然而，像上面的 `href: '/favicon.ico'` 这样的静态 `app.head` 链接只是一个字面路径，**不会**根据 `cdnURL` 解析。要让 favicon 指向解析后的地址，请在 `app.vue` 中使用 [`useHead()`](/docs/4.x/api/composables/use-head) 并从运行时配置构建 `href`：
...
你也可以提供 [类型](/docs/4.x/getting-started/seo-meta#types) 中列出的任意键。
...
useSeoMeta({
  title: '我的超棒网站',
  ogTitle: '我的超棒网站',
  description: '这是我的超棒网站，让我来为你详细介绍一下。',
  ogDescription: '这是我的超棒网站，让我来为你详细介绍一下。',
...
建议将组件包裹在 `<Head>` 或 `<Html>` 组件中，因为这样标签会以更直观的方式去重。
