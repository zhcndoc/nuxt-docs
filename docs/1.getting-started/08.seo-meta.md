...
In your [`app/pages/` directory](/docs/4.x/guide/directory-structure/app/pages), you can use `definePageMeta` together with [`useHead`](/docs/4.x/api/composables/use-head) to set metadata based on the current route.
...
The following example shows how to enable Google Fonts using the `link` property of [`useHead`](/docs/4.x/api/composables/use-head) or by using the `<Link>` component:
