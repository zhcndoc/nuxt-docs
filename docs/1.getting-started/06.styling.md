...
如果您需要在预处理文件中注入代码，比如具有颜色变量的 [sass partial](https://sass-lang.com/documentation/at-rules/use#partials)，您可以使用 vite 的 [preprocessors options](https://vite.dev/config/shared-options.html#css-preprocessoroptions) 来实现。
...
Vite 提供了一个 [实验性选项](https://vite.dev/config/shared-options.html#css-preprocessormaxworkers)，可以加快预处理器的使用。
...
您可以参考 [Vue 文档](https://vuejs.org/api/sfc-css-features.html) 来获取有关在 SFC 中样式组件的全面参考。
...
参阅 [Vue 文档](https://vuejs.org/guide/essentials/class-and-style.html) 以获取更多信息。
...
Nuxt 对于样式没有特定偏好，提供了一系列广泛的选项。您可以使用任何您想要的样式工具，例如流行的 [UnoCSS](https://unocss.dev) 或 [Tailwind CSS](https://tailwindcss.com)。
...
如果您正在使用 [UnoCSS](https://unocss.dev/integrations/nuxt)，请注意它带有 [Web Fonts Presets](https://unocss.dev/presets/web-fonts)，可以方便地从常见提供者（包括 Google Fonts 等）加载字体。
