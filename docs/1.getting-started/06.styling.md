...
Vue 和 SFC 最出色的特性之一，就是它们能够非常自然地处理样式。你可以直接在组件文件的 style 块中编写 CSS 或预处理器代码，因此无需使用 CSS-in-JS 之类的方案，也能获得出色的开发体验。不过，如果你希望使用 CSS-in-JS，可以找到支持它的第三方库和模块，例如 [pinceau](https://github.com/Tahul/pinceau)。
...
你可以在 style 块中通过 v-bind 函数引用 JavaScript 变量和表达式。
绑定是动态的，这意味着如果变量值发生变化，样式也会随之更新。
