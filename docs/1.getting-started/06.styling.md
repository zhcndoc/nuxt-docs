...
外部样式表是阻塞渲染的资源：在浏览器渲染页面之前必须加载和处理它们。包含不必要的大量样式的网页渲染时间会更长。你可以在 [web.dev](https://web.dev/defer-non-critical-css) 上阅读更多相关内容。
...
如果你需要向预处理文件注入代码，例如包含颜色变量的 [Sass partial](https://sass-lang.com/documentation/at-rules/use#partials)，可以使用 Vite 的 [preprocessor options](https://vite.zhcndoc.com/config/shared-options.html#css-preprocessoroptions)。
...
Vite 提供了一个[实验性选项](https://vite.zhcndoc.com/config/shared-options.html#css-preprocessormaxworkers)，可以加速预处理器的使用。
...
你可以参考 [Vue 文档](https://vue.zhcndoc.com/api/sfc-css-features.html) 获取有关在 SFC 中为组件添加样式的完整参考。
...
更多信息请参阅 [Vue 文档](https://vue.zhcndoc.com/guide/essentials/class-and-style.html)。
...
你可以参考 [Vite CSS 文档](https://vite.zhcndoc.com/guide/features.html#css) 和 [@vitejs/plugin-vue 文档](https://github.com/vitejs/vite-plugin-vue/tree/main/packages/plugin-vue)。
...
Nuxt 在样式方面不做强制性选择，并为你提供了多种可选方案。你可以使用任何你想要的样式工具，例如流行的库 [UnoCSS](https://unocss.dev) 或 [Tailwind CSS](https://tailwindcss.zhcndoc.com)。
...
你可以在 [web.dev](https://web.dev/optimize-lcp) 找到 LCP 优化指南。
