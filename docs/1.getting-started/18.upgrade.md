...
- **Nitro 插件导入**：使用 `import { defineNitroPlugin } from 'nitro/runtime'` 显式导入（自动导入依然有效）。
- **运行时钩子**：`nitroApp.hooks.hook('beforeResponse', ...)` 和 `nitroApp.hooks.hook('afterResponse', ...)` 已被合并为 `nitroApp.hooks.hook('response', ...)`。
- **来自 `nitro/app` 的 `getRouteRules()`**：服务器端的 Nitro 辅助函数由 `getRouteRules(event)` 改为 `getRouteRules(method, pathname)`，返回 `{ routeRules }`。
...
不再回退检测 `ssrContext._renderResponse`。现在只使用 Nuxt 路由组合函数设置的内部 `ssrContext['~renderResponse']`。

#### 变更原因

`ssrContext` 上的 `_renderResponse` 属性在 [#33896](https://github.com/nuxt/nuxt/pull/33896) 将内部 API 迁移到 `~renderResponse` 后，仅作为兼容回退保留。TODO 注释指出应在 Nuxt v5 中移除。

#### 迁移步骤

若你直接设置过 `ssrContext._renderResponse`（这从未是公开 API），请改用 `ssrContext['~renderResponse']`。Nuxt 路由组合函数已经使用新属性，若你通过 `navigateTo` 或路由中间件，不需改动。
...
`experimental.renderJsonPayloads` 选项已被移除。现在 JSON 负载（`_payload.json`）是唯一支持的负载格式。脚本类型负载（`_payload.js`）不再可用，同时 `@nuxt/devalue` 依赖也已被移除。

#### 变更原因

自 Nuxt 3.4 起，JSON 负载成为默认格式，且具有更好的性能和安全性。维护两种负载格式增加了不必要的复杂性。

#### 迁移步骤

如果你的 `nuxt.config.ts` 中设置了 `experimental.renderJsonPayloads`，请将其移除：
...
如果你的应用依赖于脚本类型负载（`_payload.js`），现在将自动使用 JSON 负载（`_payload.json`），无需其他更改。
...
- `experimental.renderJsonPayloads` 已移除（参见 [删除 Script-Type Payloads](/docs/5.x/getting-started/upgrade#removal-of-script-type-payloads)）
