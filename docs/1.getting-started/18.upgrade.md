...
- **Vite Environment API**：使用新的 [Vite Environment API](/docs/4.x/getting-started/upgrade#migration-to-vite-environment-api) 改进构建配置
- **区分大小写的路由**：页面路由[完全匹配 URL 的大小写](/docs/4.x/getting-started/upgrade#case-sensitive-routing)，与 Nitro 保持一致
- **规范化页面名称**：页面组件名称将[匹配其路由名称](/docs/4.x/getting-started/upgrade#normalized-page-component-names)，以确保 `<KeepAlive>` 行为一致
- **`clearNuxtState` 重置为默认值**：`clearNuxtState` 将[把状态重置为初始值](/docs/4.x/getting-started/upgrade#respect-defaults-when-clearing-usestate)，而不是将其设置为 `undefined`
- **非异步 `callHook`**：[`callHook` 可能返回 `void`](/docs/4.x/getting-started/upgrade#non-async-callhook)，而不是始终返回 `Promise`
- **注释节点占位符**：仅客户端组件使用[注释节点而不是 `<div>`](/docs/4.x/getting-started/upgrade#client-only-comment-placeholders)作为 SSR 占位符，从而修复作用域样式激活问题
...
- **Vue Options API 已禁用**：[Options API 会从客户端 bundle 中被编译移除](/docs/4.x/getting-started/upgrade#vue-options-api-disabled-by-default)，以减小其体积
- **已移除 `process.*` 类型增强**：TypeScript 不再在 `NodeJS.Process` 上暴露[已弃用的 `process.*` 标志](/docs/4.x/getting-started/upgrade#process-type-augmentation-removed)
- **类型化页面**：[`experimental.typedPages`](/docs/4.x/getting-started/upgrade#typed-pages-enabled-by-default) 默认启用，以支持类型检查路由
