...
Nuxt 使用了 [unjs/c12](https://github.com/unjs/c12) 和 [unjs/giget](https://github.com/unjs/giget) 来扩展远程图层。请查看文档以获取更多信息和所有可用选项。
...
layers/
  1.base/
    components/Button.vue    # 基础按钮样式
  2.theme/
    components/Button.vue    # 主题按钮（覆盖基础）
app/
  components/Button.vue          # 项目按钮（覆盖所有图层）
```

在这种情况下：  
- 如果只有图层存在，使用 `2.theme/Button.vue`（字母序较高）  
- 如果项目中存在 `components/Button.vue`，则覆盖所有图层

### 控制优先级

你可以通过给图层目录加数字前缀来控制顺序：
...
::tip
这种模式适用于创建带有默认配置的基础图层，然后由更具体的图层逐步覆盖。
::

### 何时使用各自方式

- **`~~/layers` 目录** - 用于作为项目一部分的本地图层  
- **`extends`** - 用于外部依赖（npm 包、远程仓库）或项目目录外的图层
...
了解 **layers/ 目录**，用于组织和共享可复用的代码、组件、组合函数和配置，贯穿你的 Nuxt 应用。
::

::read-more{to="/docs/3.x/guide/going-further/layers"}
在 **图层作者指南** 中阅读关于图层的更多内容。
::
