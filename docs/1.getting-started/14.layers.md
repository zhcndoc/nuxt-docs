...
或者，你也可以通过在 [`extends`](/docs/4.x/api/nuxt-config#extends) 中引用它们，从 `nuxt.config` 控制 `~~/layers` 目录的顺序，而无需重命名目录。条目会像其他 `extends` 图层一样排序，其中**第一个条目具有最高优先级**：
...
同时支持 `~~/...`（推荐）和 `~/...` 别名形式，以及相对路径（`./layers/admin`）。任何你未列出的 `~~/layers` 中的图层都会保留其按字母顺序自动扫描的顺序，并排在你在 `extends` 中列出的图层之后。
