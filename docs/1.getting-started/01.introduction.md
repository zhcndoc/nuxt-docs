...
:read-more{title="Nuxt 渲染模式" to="/docs/3.x/guide/concepts/rendering"}
...
:read-more{title="Nuxt 服务器引擎" to="/docs/3.x/guide/concepts/server-engine"}

### 生产就绪

Nuxt 应用可部署到 Node 或 Deno 服务器，预渲染后托管于静态环境，或部署至 Serverless 与边缘服务提供商。

:read-more{title="部署章节" to="/docs/3.x/getting-started/deployment"}

### 模块化

模块系统允许通过自定义功能和第三方服务集成扩展 Nuxt。

:read-more{title="Nuxt 模块概念" to="/docs/3.x/guide/concepts/modules"}
