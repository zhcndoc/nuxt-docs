...
您可以使用 [`onErrorCaptured`](https://vue.zhcndoc.com/api/composition-api-lifecycle.html#onerrorcaptured) 钩子捕获 Vue 错误。

此外，Nuxt 提供了一个 [`vue:error`](/docs/api/advanced/hooks#app-hooks-runtime) 钩子，如果任何错误传播到顶层，将会被调用。

如果您使用错误报告框架，可以通过 [`vueApp.config.errorHandler`](https://vue.zhcndoc.com/api/application.html#app-config-errorhandler) 提供全局处理程序。即使错误被处理，它也会接收所有 Vue 错误。
