...
Nuxt 配备了两个组合式函数和一个内置库，用于在浏览器或服务器环境中执行数据获取：`useFetch`、[`useAsyncData`](/docs/api/composables/use-async-data) 和 `$fetch`。

简而言之：

- [`$fetch`](/docs/api/utils/dollarfetch) 是发起网络请求的最简单方式。
- [`useFetch`](/docs/api/composables/use-fetch) 是 `$fetch` 的封装，确保在[通用渲染](/docs/guide/concepts/rendering#universal-rendering)过程中只获取一次数据。
- [`useAsyncData`](/docs/api/composables/use-async-data) 类似于 `useFetch`，但提供了更细粒度的控制。

`useFetch` 和 `useAsyncData` 共享一套通用的选项和模式，我们将在最后的章节详细介绍。

## 为什么需要 `useFetch` 和 `useAsyncData`

Nuxt 是一个能够在服务器和客户端环境中运行同构（或通用）代码的框架。如果在 Vue 组件的 setup 函数中直接使用 [`$fetch` 函数](/docs/api/utils/dollarfetch) 来执行数据获取，可能会导致数据被获取两次：一次在服务器端（用于渲染 HTML），一次在客户端（HTML 水合时）。这会引发水合问题，增加交互时间，且可能导致不可预测的行为。

[`useFetch`](/docs/api/composables/use-fetch) 和 [`useAsyncData`](/docs/api/composables/use-async-data) 通过确保如果在服务器上调用了 API，则数据会通过负载转发给客户端，从而解决了这个问题。

负载是一个 JavaScript 对象，通过 [`useNuxtApp().payload`](/docs/api/composables/use-nuxt-app#payload) 访问。它在客户端用于避免在浏览器执行代码[水合期间](/docs/guide/concepts/rendering#universal-rendering)重复获取相同数据。
...
::note
你可以添加 [`<NuxtLoadingIndicator>`](/docs/api/components/nuxt-loading-indicator) 来在页面导航间显示进度条。
::
...
::warning
仅使用 `$fetch` 不会提供[网络请求去重和导航阻止](#为什么需要-usefetch-和-useasyncdata)。:br
建议对于客户端交互（基于事件的请求）使用 `$fetch`，或在获取初始组件数据时与 [`useAsyncData`](#useasyncdata) 结合使用。
::
...
服务端调用 `useFetch` 时，Nuxt 会使用 [`useRequestFetch`](/docs/api/composables/use-request-fetch) 代理客户端请求头和 Cookie（除了不应转发的请求头，如 `host`）。
...
另外，下面示例展示如何使用 [`useRequestHeaders`](/docs/api/composables/use-request-headers) 访问并传递服务端请求中的 Cookie（来自客户端请求）。通过同构的 `$fetch` 调用，我们确保 API 端点可以访问用户浏览器原始发送的 `cookie` 头。这仅在不使用 `useFetch` 时才需要。
...
::tip
你也可以使用 [`useRequestFetch`](/docs/api/composables/use-request-fetch) 来自动代理请求头。
::
...
[`useFetch`](/docs/api/composables/use-fetch) 组合函数基于 `$fetch`，用于在 setup 函数中安全地执行 SSR 网络请求。
...
此组合函数是对 [`useAsyncData`](/docs/api/composables/use-async-data) 组合函数和 `$fetch` 工具的封装。
...
它是最常用场景的开发体验简化版。（你可以在[`useRequestFetch`](/docs/api/composables/use-request-fetch)了解更多关于 `event.fetch` 的信息。）
::

:video-accordion{title="观看 Alexander Lichter 的视频，深入了解 useFetch 和 useAsyncData 的区别" videoId="0X-aOpSGabA"}

有些场景不适合使用 [`useFetch`](/docs/api/composables/use-fetch)，例如当 CMS 或第三方提供自己的查询层时。这时可以用 [`useAsyncData`](/docs/api/composables/use-async-data) 来包装你的调用，同时保留组合函数提供的优势。
...
::note
[`useAsyncData`](/docs/api/composables/use-async-data) 的第一个参数是唯一键，用于缓存第二个参数（查询函数）的响应。若直接传递查询函数，则该键将自动生成。
:br :br
由于自动生成的键仅考虑了 `useAsyncData` 调用所在文件和行号，建议始终自定义唯一键以避免不期望的行为，尤其是当你创建自己的自定义组合函数包装 `useAsyncData` 时。
:br :br
设置键还有助于通过 [`useNuxtData`](/docs/api/composables/use-nuxt-data) 共享同一数据或[刷新特定数据](/docs/api/utils/refresh-nuxt-data#refresh-specific-data)。
::
...
::note
`useAsyncData` 用于获取和缓存数据，而非触发副作用（如调用 Pinia action），因为这可能导致重复执行且产生 nullish 值等问题。如果需要触发副作用，请使用 [`callOnce`](/docs/api/utils/call-once) 工具。
...
- `clear`: 用于将 `data` 设为 `undefined`，`error` 设为 `null`，`status` 设为 `idle`，并将当前所有待定请求标记为取消。
- `error`: 数据获取失败时的错误对象。
...
[`useAsyncData`](/docs/api/composables/use-async-data) 和 [`useFetch`](/docs/api/composables/use-fetch) 返回相同类型的对象，接受一组通用选项作为最后一个参数，帮助你控制组合函数的行为，如导航阻止、缓存或执行等。
...
你也可以使用 [`useLazyFetch`](/docs/api/composables/use-lazy-fetch) 和 `useLazyAsyncData` 来更便捷地实现同样效果。
...
[`useFetch`](/docs/api/composables/use-fetch) 和 [`useAsyncData`](/docs/api/composables/use-async-data) 使用键来防止重复获取相同数据。

- [`useFetch`](/docs/api/composables/use-fetch) 使用提供的 URL 作为键。也可通过最后一个参数的 `key` 选项提供自定义键。
- [`useAsyncData`](/docs/api/composables/use-async-data) 如果第一个参数是字符串，则作为键。若第一个参数是查询函数，则自动基于文件名与行号生成唯一键。

::tip
可以使用 [`useNuxtData`](/docs/api/composables/use-nuxt-data) 通过键获取缓存数据。
::
...
::tip
想全局重新获取或使缓存失效，参见 [`clearNuxtData`](/docs/api/utils/clear-nuxt-data) 和 [`refreshNuxtData`](/docs/api/utils/refresh-nuxt-data)。
::
...
但在服务器上调用相对 URL 的 `useFetch` 时，Nuxt 会使用 [`useRequestFetch`](/docs/api/composables/use-request-fetch) 代理请求头和 Cookie（除不应转发的头，如 `host`）。
...
使用 `useAsyncData` 和 `useLazyAsyncData` 将服务器获取的数据传递到客户端（以及其他使用[Nuxt 负载](/docs/api/composables/use-nuxt-app#payload)的场景）时，负载会通过 [`devalue`](https://github.com/Rich-Harris/devalue) 来序列化。这样我们不仅能传递基础 JSON 类型，还可序列化并恢复更高级的数据类型，如正则表达式、日期、Map 和 Set、`ref`、`reactive`、`shallowRef`、`shallowReactive` 和 `NuxtError` 等。

也可以为 Nuxt 不支持的类型定义自己的序列化/反序列化方法。具体请参考 [`useNuxtApp`](/docs/api/composables/use-nuxt-app#payload) 文档。
...
从 `server` 目录获取数据时，响应会使用 `JSON.stringify` 序列化。但由于序列化只支持 JavaScript 原始类型，Nuxt 会尽量转换 `$fetch` 和 [`useFetch`](/docs/api/composables/use-fetch) 返回的数据以匹配实际值。
...
::tip
若通过 GET 请求消费 SSE，可使用 [`EventSource`](https://developer.mozilla.org/en-US/docs/Web/API/EventSource) 或 VueUse 组合式函数 [`useEventSource`](https://vueuse.zhcndoc.com/core/useEventSource/)。
::
