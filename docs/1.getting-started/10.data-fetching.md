...
使用 [`useFetch`](/docs/4.x/api/composables/use-fetch) composable 并不总是合适，例如 CMS 或第三方提供了自己的查询层。在这种情况下，您可以使用 [`useAsyncData`](/docs/4.x/api/composables/use-async-data) 包装您的调用，同时保留该 composable 提供的优势。
