...
Nuxt 在内部使用 Vue 的 [`<Suspense>`](https://vuejs.org/guide/built-ins/suspense) 组件，防止在每次异步数据可用于视图之前导航。数据获取组合函数可以帮助您利用此功能，并根据每次调用使用最适合的函数。
...
在更复杂的 URL 构建场景中，您可以使用回调作为 [计算 getter](https://vuejs.org/guide/essentials/computed.html)，返回 URL 字符串。
...
::tip
如果通过 GET 请求消费 SSE，您可以使用 [`EventSource`](https://developer.mozilla.org/en-US/docs/Web/API/EventSource) 或 VueUse 组合函数 [`useEventSource`](https://vueuse.org/core/useEventSource/)。
::
