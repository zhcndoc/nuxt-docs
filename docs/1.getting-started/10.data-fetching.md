...
本 документации 中的示例通常会对 `useFetch` 和 `useAsyncData` 的调用使用 `await`，但这并不总是必需的。
...
在某些情况下，使用 [`useFetch`](/docs/4.x/api/composables/use-fetch) 并不合适，例如当 CMS 或第三方提供自己的查询层时。在这种情况下，您可以使用 [`useAsyncData`](/docs/4.x/api/composables/use-async-data) 来包装调用，同时仍保留该组合式函数提供的好处。
...
::tip
If you consume SSE via GET request, you can use [`EventSource`](https://developer.mozilla.org/en-US/docs/Web/API/EventSource) or VueUse's composable [`useEventSource`](https://vueuse.org/core/useEventSource/).
::
...
:video-accordion{title="Watch a Vue School video to learn about parallel data fetching" videoId="1024262536" platform="vimeo"}
